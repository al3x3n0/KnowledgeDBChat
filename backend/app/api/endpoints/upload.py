    ...
    try:
        ...
        if not validate_file_type(filename, content_type):
            raise ValidationError(
                f"File type not allowed: {content_type}", field="file"
            )
        ...
        logger.info(
            f"Created upload session {session.id} for file {filename} ({file_size} bytes, {total_chunks} chunks)"
        )
...
async def get_upload_status(
    ...
    result = await db.execute(
        select(UploadSession).where(
            UploadSession.id == session_id, UploadSession.user_id == current_user.id
        )
    )
    ...
    try:
        ...
        if not session.minio_upload_id:
            ...
            session.extra_metadata[
                "chunk_paths"
            ] = {}  # Store individual chunk paths for manual reassembly
            session.extra_metadata["use_multipart"] = not upload_id.startswith(
                "upload_"
        ...
        if use_multipart and not session.minio_upload_id.startswith("upload_"):
            ...
            etag = await storage_service.upload_part(
                object_path, session.minio_upload_id, part_number, chunk_data
            )
        else:
        ...
        logger.info(
            f"Uploaded chunk {chunk_number}/{session.total_chunks - 1} for session {session_id} ({session.uploaded_bytes}/{session.file_size} bytes)"
        )
    ...
    try:
        ...
        if session.status == "completed":
            if session.document_id:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Upload marked as completed but no document ID found",
                )
        ...
        if not object_path or not session.minio_upload_id:
            raise HTTPException(
                status_code=400, detail="Multipart upload not initialized"
            )
        ...
        if use_multipart and not session.minio_upload_id.startswith("upload_"):
            ...
            await storage_service.complete_multipart_upload(
                object_path, session.minio_upload_id, parts
            )

            logger.info(
                f"Completed MinIO multipart upload for session {session_id} - file automatically reassembled"
            )
        else:
            # Manual reassembly: download all chunks and concatenate them
            logger.info(
                f"Reassembling file manually from chunks for session {session_id}"
            )
            import shutil
            import tempfile
            ...
            try:
                ...
                with open(temp_file_path, "wb") as final_file:
                    for chunk_idx in range(session.total_chunks):
                        chunk_path = session.extra_metadata.get("chunk_paths", {}).get(
                            str(chunk_idx)
                        )
                        if not chunk_path:
                            raise HTTPException(
                                status_code=400,
                                detail=f"Chunk {chunk_idx} path not found",
                            )
                ...
                for chunk_path in session.extra_metadata.get(
                    "chunk_paths", {}
                ).values():
                    try:
        ...
        if not upload_source:
            upload_source = DocumentSource(
                name="File Upload", source_type="file", config={}, is_active=True
            )
        ...
        final_object_path = storage_service._get_object_path(
            document.id, session.filename
        )
        if object_path != final_object_path:
            # Copy file to final location (MinIO doesn't have move, so we copy and delete)
            logger.info(
                f"Copying file from temp path {object_path} to final path {final_object_path}"
            )
            ...
            if not copy_success:
                raise HTTPException(
                    status_code=500,
                    detail="Failed to copy file from temporary location to final location",
                )
        ...
        if transcription_service:
            ...
            final_ext = Path(document.file_path).suffix or (
                Path(session.filename).suffix if "session" in locals() else ""
            )
            try:
                temp_named = tempfile.NamedTemporaryFile(
                    delete=False, suffix=final_ext or ".tmp"
                )
                temp_named.close()
                ...
                if transcription_service.is_supported_format(file_path_obj):
                    # Video/audio file
                    logger.info(
                        f"Video/audio file detected: {session.filename}. Scheduling tasks."
                    )
                    document.content = ""
                    ...
                    is_video = ext in video_exts or (
                        session.file_type or ""
                    ).startswith("video/")
                    is_audio = ext in audio_exts or (
                        session.file_type or ""
                    ).startswith("audio/")
                    ...
                    if is_video and not is_mp4:
                        ...
                        logger.info(
                            f"Triggered transcription task for document {document.id} with soft_limit={soft_limit}s hard_limit={hard_limit}s"
                        )
                else:
                    ...
                    (
                        text_content,
                        extraction_metadata,
                    ) = await text_processor.extract_text(
                        temp_check_path, session.content_type
                    )
        ...
        logger.info(
            f"Completed upload session {session_id}, created document {document.id}"
        )
        ...
        try:
            ...
            if object_path:
                await storage_service.abort_multipart_upload(
                    object_path, session.minio_upload_id
                )
        except Exception as e:
