...
@celery_app.task(
    bind=True, name="app.tasks.paper_enrichment_tasks.enrich_arxiv_document"
)
def enrich_arxiv_document(
...
def enrich_arxiv_source(
    self, source_id: str, force: bool = False, limit: int = 500
) -> Dict[str, Any]:
    return asyncio.run(_async_enrich_source(self, source_id, force, limit))


async def _async_enrich_source(
    task, source_id: str, force: bool, limit: int
) -> Dict[str, Any]:
    async with create_celery_session()() as db:
        try:
            ...
            if not src or src.source_type != "arxiv":
                return {
                    "success": False,
                    "source_id": source_id,
                    "error": "Source not found or not arXiv",
                }
