...
from app.services.research_opportunity_service import list_normalized_research_opportunities
...
def summarize_follow_up_job_outcome(job: AgentJob) -> str:
    ...
    if status == AgentJobStatus.COMPLETED.value:
        for candidate in (
            ...
            if summary:
                return summary
        return f"{str(job.name or 'Follow-up job').strip() or 'Follow-up job'} completed."

    if status == AgentJobStatus.FAILED.value:
        summary = _normalize_summary_text(job.error) or _normalize_summary_text(job.phase_details)
        if summary:
            return summary
        return f"{str(job.name or 'Follow-up job').strip() or 'Follow-up job'} failed."

    if status == AgentJobStatus.CANCELLED.value:
        summary = _normalize_summary_text(job.phase_details) or _normalize_summary_text(job.error)
        if summary:
            return summary
...
def _resolve_follow_up_opportunity_origin(job: AgentJob) -> tuple[str | None, str | None, str | None]:
    job_config = getattr(job, "config", None)
    config = job_config if isinstance(job_config, dict) else {}
    follow_up = config.get("domain_research_follow_up") if isinstance(config.get("domain_research_follow_up"), dict) else {}
    idea = follow_up.get("idea") if isinstance(follow_up.get("idea"), dict) else {}
    metadata = config.get("autonomy_metadata") if isinstance(config.get("autonomy_metadata"), dict) else {}
    origin = idea.get("autonomous_origin") if isinstance(idea.get("autonomous_origin"), dict) else {}
    opportunity_id = (
        str(origin.get("opportunity_id") or "").strip()
        or str(metadata.get("profile_opportunity_id") or metadata.get("portfolio_opportunity_id") or "").strip()
        or str(idea.get("opportunity_id") or "").strip()
    )
    source_kind = (
        str(origin.get("source_kind") or "").strip().lower()
        or ("profile" if str(metadata.get("domain_research_profile_id") or "").strip() else "portfolio" if str(metadata.get("portfolio_id") or "").strip() else "")
    )
    source_id = (
        str(origin.get("source_id") or "").strip()
        or str(metadata.get("domain_research_profile_id") or metadata.get("portfolio_id") or "").strip()
    )
...
async def _project_follow_up_outcome_to_originating_opportunity(
    ...
    *,
    ...
    if source_kind == "profile":
        ...
        if profile is None:
            return False
        profile_summary = dict(profile.latest_summary) if isinstance(profile.latest_summary, dict) else {}
        rows = list_normalized_research_opportunities(profile_summary.get("opportunities") or profile_summary.get("idea_candidates") or [])
        idx = next((i for i, row in enumerate(rows) if str(row.get("opportunity_id") or "").strip() == opportunity_id), -1)
        if idx < 0:
            return False
        row = dict(rows[idx])
        ...
        if not str(row.get("follow_up_launched_at") or "").strip():
            ...
            row["follow_up_launched_at"] = launched_at.isoformat() if launched_at else next_recorded_at
...
async def project_follow_up_relaunch_to_originating_opportunity(
    ...
    *,
    ...
    def _apply_relaunch(row: dict) -> bool:
        ...
        child_job_ids = [
            str(value or "").strip()
            for value in (row.get("child_job_ids") if isinstance(row.get("child_job_ids"), list) else [])
        ...
        row_changed = (
            ...
            or str(row.get("last_decision_reason_code") or "").strip() != "follow_up_relaunched"
...
def _build_follow_up_outcome_notification_content(
    ...
    *,
    ...
    if status == AgentJobStatus.FAILED.value:
        ...
        message = summary or "A launched follow-up failed and may need operator relaunch."
        priority = "high"
    elif status == AgentJobStatus.CANCELLED.value:
        title = f"Follow-up cancelled: {title_text[:120]}"
        message = summary or "A launched follow-up was cancelled and can be relaunched from the inbox."
        priority = "high"
    else:
...
async def _maybe_emit_follow_up_outcome_notification(
    ...
    *,
    ...
    existing_result = await db.execute(
        ...
        .where(
            and_(
                ...
                Notification.notification_type == NotificationType.FOLLOW_UP_OUTCOME_ALERT,
    ...
    for notification in existing_notifications:
        ...
        if (
            str(data.get("follow_up_outcome_status") or "").strip().lower() == status
            and str(data.get("follow_up_job_id") or "").strip() == str(job.id)
        ):
            return
