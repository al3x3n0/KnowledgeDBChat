...
class MemoryService:
    ...
    async def create_memory(
        self, user_id: UUID, memory_data: MemoryCreate, db: AsyncSession
    ) -> MemoryResponse:
    ...
    async def get_memory_by_id(
        self, memory_id: UUID, user_id: UUID, db: AsyncSession
    ...
    async def search_memories(
        self, user_id: UUID, search_request: MemorySearchRequest, db: AsyncSession
    ) -> List[MemoryResponse]:
        """Search memories using semantic similarity."""
        try:
            # Get user preferences for search
            await self.get_user_preferences(user_id, db)
            ...
            if search_request.session_id is not None:
                conditions.append(
                    ConversationMemory.session_id == search_request.session_id
                )

            # Add importance score filter only if min_importance is not None
            if search_request.min_importance is not None:
                conditions.append(
                    ConversationMemory.importance_score >= search_request.min_importance
                )
            ...
            if search_request.memory_types:
                query = query.where(
                    ConversationMemory.memory_type.in_(search_request.memory_types)
                )
            ...
            if search_request.tags:
                query = query.where(
                    or_(
                        *[
                            ConversationMemory.tags.contains([tag])
    ...
    async def extract_memories_from_conversation(
        ...
        try:
            ...
            logger.info(
                f"Extracted {len(created_memories)} memories from session {session_id}"
            )
    ...
    async def generate_memory_summary(
        self, user_id: UUID, summary_request: MemorySummaryRequest, db: AsyncSession
    ) -> MemorySummaryResponse:
        ...
        try:
            ...
            return MemorySummaryResponse(
                ...
                context_items=[
                    m.content for m in context_items[:10]
    ...
    async def get_memory_stats(
        ...
        try:
            ...
            type_result = await db.execute(
                select(
                    ConversationMemory.memory_type, func.count(ConversationMemory.id)
                )
                .where(
                    and_(
                        ConversationMemory.user_id == user_id,
            ...
            most_accessed = [
                MemoryResponse.from_orm(m) for m in accessed_result.scalars().all()
            ]
    ...
    async def _find_similar_memory(
        self, user_id: UUID, content: str, memory_type: str, db: AsyncSession
    ...
    async def _get_memory_usage_trend(
        ...
        try:
            ...
            for i in range(30):
                ...
                trend_data.append(
                    {"date": start_date.strftime("%Y-%m-%d"), "count": count}
                )
