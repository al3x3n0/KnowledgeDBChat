...
async def _async_transcode_to_mp4(task, document_id: str) -> Dict[str, Any]:
    async with create_celery_session()() as db:
        ...
        if meta.get("is_transcoded") and meta.get("stream_file_path"):
            logger.info(
                f"Transcode: document {document_id} already transcoded -> {meta.get('stream_file_path')}"
            )
        ...
        if not is_video:
            logger.info(
                f"Transcode: document {document_id} is not a video (ext={ext}), skipping"
            )
            return {"success": True, "document_id": document_id, "skipped": True}
        ...
        if not needs_transcode:
            ...
            document.extra_metadata = {
                **meta,
                "is_transcoded": True,
                "stream_file_path": document.file_path,
            }
            await db.commit()
            return {"success": True, "document_id": document_id, "skipped": True}
        ...
        try:
            ...
            with open(temp_dst, "rb") as f:
                data = f.read()
            new_object_path = await storage_service.upload_file(
                UUID(str(document.id)), new_filename, data, "video/mp4"
            )
            ...
            _publish_progress(
                ...
                {
                    "stage": "transcoding",
                    "message": "Transcode complete. Starting transcription...",
                    "progress": 100,
                },
            )
            _publish_status(
            ...
            try:
                ...
                logger.info(
                    f"Transcode: dispatched transcription for document {document_id} with soft_limit={soft_limit}s hard_limit={hard_limit}s"
                )
