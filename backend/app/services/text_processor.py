...
class TextProcessor:
    ...
    def __init__(self):
        ...
        # Loading sentence-transformers can download model files and must not block
        # API module imports. Initialize it lazily on the first semantic split.
        self.semantic_model = None
        self._semantic_model_load_attempted = False

    async def _ensure_semantic_model(self) -> bool:
        """Load the semantic chunking model on demand without blocking the event loop."""
        ...
        try:
            ...
            logger.warning(
                f"Failed to initialize semantic chunking model: {e}. Falling back to fixed-size chunking."
            )
            self.semantic_model = None
            return False
    ...
    async def extract_text(
        ...
        try:
            ...
            elif file_extension in [".docx", ".doc"] or (
                content_type and "word" in content_type
            ):
                text = await self._extract_word(file_path)
            elif file_extension in [".pptx", ".ppt"] or (
                content_type
                and (
            ...
            elif file_extension in [".html", ".htm"] or (
                content_type and "html" in content_type
            ):
                text = await self._extract_html(file_path)
            elif file_extension in [".md", ".markdown"] or (
                content_type and "markdown" in content_type.lower()
            ):
    ...
    async def _extract_pdf(self, file_path: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"Extracted {text_length} characters from {page_count} pages of PDF: {file_path}"
            )

            return text_content
        except ImportError as e:
            logger.error(
                f"PDF extraction library not available: {e}. Please install pypdf."
            )
            raise
        except Exception as e:
        ...
        try:
            ...
            for idx, slide in enumerate(presentation.slides, start=1):
                ...
                for shape in slide.shapes:
                    ...
                    try:
                        if title_text is None and getattr(
                    ...
                    if comments_part is not None:
                        for comment in getattr(comments_part, "comment_list", []):
                            ...
                            try:
                                ...
                                author_initials = (
                                    getattr(author_obj, "initials", None)
                                    if author_obj
                                    else None
                                )
                            except Exception:
                            ...
                            timestamp = None
                            comment_dt = getattr(comment, "datetime", None) or getattr(
                                comment, "dt", None
                            )
                            if comment_dt:
                                try:
            ...
            if slide_sections:
                ...
                metadata = {
                    "presentation": {
                        ...
                        "has_comments": any(
                            entry.get("comments") for entry in slide_entries
                        ),
                    }
                }
                text_length = len(text_content)
                logger.info(
                    f"Extracted {text_length} characters from {len(slide_entries)} slides of PowerPoint: {file_path}"
                )
            ...
            metadata = {"presentation": {"slide_count": len(documents)}}
            return text_content, metadata
        except Exception as e:
    ...
    async def _extract_markdown(self, file_path: str) -> str:
        ...
        logger.info(
            f"Extracted {text_length} characters from Markdown: {file_path} "
    ...
    def _extract_section_title(self, text: str, position: int) -> Optional[str]:
        ...
        html_headers = re.findall(
            r"<h[1-6][^>]*>(.+?)</h[1-6]>", before_text, re.IGNORECASE | re.DOTALL
        )
        if html_headers:
            # Clean HTML tags
    ...
    async def split_text(
        ...
        try:
            if strategy == "semantic" and await self._ensure_semantic_model():
    ...
    async def _fixed_chunk(
        ...
        if (
            ...
            or chunk_overlap != self.text_splitter._chunk_overlap
        ):
    ...
    async def _semantic_chunk(
        ...
        if not self.semantic_model:
            logger.warning(
                "Semantic model not available, falling back to fixed chunking"
            )
        ...
        try:
            ...
            if len(chunks) < 2 and len(cleaned_text) > max_chunk_size:
                chunks = await self._semantic_chunk_by_sentences(
                    cleaned_text, max_chunk_size, overlap
                )
            ...
            logger.error(
                f"Error in semantic chunking: {e}, falling back to fixed chunking"
            )
            return await self._fixed_chunk(text, max_chunk_size, overlap)
    ...
    async def _semantic_chunk_by_sentences(
        ...
        for i, sentence in enumerate(sentences):
            ...
            if (similarity < 0.7 and current_chunk) or (
