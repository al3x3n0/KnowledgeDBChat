...
def _profile_to_quick_start_request(
    ...
    return AgentJobQuickStartDomainResearchRequest(
        ...
        if isinstance(profile.scoring_policy, dict)
        else None,
        selection_policy=profile.selection_policy
        if isinstance(profile.selection_policy, dict)
        else None,
        persist_artifacts=bool(profile.persist_artifacts),
        auto_launch_follow_up=bool(
            effective_policy.get("auto_launch_follow_up", profile.auto_launch_follow_up)
        ),
...
async def _validate_sandbox_profile(
    ...
    *,
    ...
    profile = await get_scientific_sandbox_profile(
        db, requested, track_type=track_type, include_disabled=False
    )
    if not isinstance(profile, dict):
        raise HTTPException(
            status_code=400, detail="Unknown or disabled sandbox profile"
        )
...
async def _create_profile_job(
    *,
    ...
    config["auto_create_experiment_plans"] = bool(
        effective_policy.get(
            "auto_create_experiment_plans", profile.auto_create_experiment_plans
        )
    )
...
async def _profile_response(
    profile: DomainResearchProfile, db: AsyncSession
) -> DomainResearchProfileResponse:
    automation_profile, effective_policy = resolve_domain_profile_automation_contract(
        automation_profile=getattr(profile, "automation_profile", None),
        automation_policy=getattr(profile, "automation_policy", None),
    ...
    typed_summaries = (
        [
            ScientificValidationRunSummaryResponse.model_validate(summary)
    ...
    validation_status_by_id = build_validation_status_map(
        latest_validation_runs=[row.model_dump() for row in typed_summaries]
        if typed_summaries
        else [],
        summary_validation_runs=summary.get("validation_runs")
...
def _find_opportunity(
    rows: list[dict[str, Any]], opportunity_id: str
...
async def _resolve_profile_parent_job(
...
async def _create_plan_for_opportunity(
    *,
    ...
    for plan in recent_plans:
        details = (
            plan.generator_details if isinstance(plan.generator_details, dict) else {}
        )
        if (
            target_opportunity_id
            and str(details.get("opportunity_id") or "").strip()
            == target_opportunity_id
        ):
            return str(plan.id)
        if (
            target_title
            and str(details.get("idea_title") or "").strip().lower() == target_title
        ):
            return str(plan.id)
    note = None
    ...
    if note is None:
        raise HTTPException(
            status_code=400, detail="Opportunity is missing a linked research note"
        )
    plan = ExperimentPlan(
        user_id=user_id,
        ...
        plan={
            ...
            "supporting_evidence": generator_details.get("supporting_evidence")
            if isinstance(generator_details.get("supporting_evidence"), list)
            else [],
            "selected_hypothesis_ids": [target_opportunity_id]
            if target_opportunity_id
            else [],
            "provenance": {
                "autonomous_origin": {
                    "source_kind": "profile",
                    "source_id": str(generator_details.get("profile_id") or "").strip()
                    or None,
                    "opportunity_id": target_opportunity_id or None,
                    "evidence_revision_at_launch": str(
                        generator_details.get("evidence_revision_at_launch") or ""
                    ).strip()
                    or None,
                }
            },
        ...
        generator=generator,
        generator_details={
            k: v for k, v in generator_details.items() if k != "existing_plan_ids"
        },
    )
    db.add(plan)
    await db.flush()
...
async def _sync_profile_opportunities(
...
async def list_domain_research_profiles(
    ...
    stmt = select(DomainResearchProfile).where(
    ...
    stmt = (
        stmt.order_by(desc(DomainResearchProfile.updated_at))
        .offset(offset)
        .limit(limit)
    )
    rows = list((await db.execute(stmt)).scalars().all())
    total_stmt = select(DomainResearchProfile.id).where(
        DomainResearchProfile.user_id == current_user.id
    )
    if str(status_filter or "").strip():
        total_stmt = total_stmt.where(
            DomainResearchProfile.status == str(status_filter).strip()
        )
...
@router.post(
    "",
    response_model=DomainResearchProfileResponse,
...
async def create_domain_research_profile(
    ...
    profile = DomainResearchProfile(
        ...
        auto_launch_follow_up=bool(
            automation_policy.get(
                "auto_launch_follow_up", payload.auto_launch_follow_up
            )
        ),
        auto_create_experiment_plans=bool(
            automation_policy.get(
                "auto_create_experiment_plans", payload.auto_create_experiment_plans
            )
        ),
        confidence_threshold=float(
            automation_policy.get("confidence_threshold", payload.confidence_threshold)
        ),
    ...
    if payload.start_immediately:
        await _create_profile_job(
            db=db, profile=profile, schedule_type="continuous", start_immediately=True
        )
        await db.refresh(profile)
...
async def update_domain_research_profile(
    ...
    (
        ...
        automation_profile=updates.get(
            "automation_profile", getattr(profile, "automation_profile", None)
        ),
        automation_policy=updates.get(
            "automation_policy", getattr(profile, "automation_policy", None)
        ),
        current_snapshot=current_domain_profile_policy_snapshot(profile),
    ...
    await _validate_sandbox_profile(
        db,
        sandbox_profile_id=updates.get(
            "sandbox_profile_id", profile.sandbox_profile_id
        ),
    ...
    updates["validation_policy"] = build_domain_profile_compat_policy(
        next_automation_policy
    )
    updates["auto_launch_follow_up"] = bool(
        next_automation_policy.get(
            "auto_launch_follow_up", profile.auto_launch_follow_up
        )
    )
    updates["auto_create_experiment_plans"] = bool(
        next_automation_policy.get(
            "auto_create_experiment_plans", profile.auto_create_experiment_plans
        )
    )
    updates["confidence_threshold"] = float(
        next_automation_policy.get(
            "confidence_threshold", profile.confidence_threshold or 0.7
        )
    )
...
async def act_on_domain_research_profile(
    ...
    active_job = (
        await db.get(AgentJob, profile.active_job_id) if profile.active_job_id else None
    )
    ...
    if action == "start":
        ...
        await _create_profile_job(
            db=db, profile=profile, schedule_type="once", start_immediately=True
        )
    else:
...
@router.post(
    "/{profile_id}/opportunities/{opportunity_id}/action",
    response_model=DomainResearchProfileResponse,
)
async def act_on_domain_research_opportunity(
    ...
    if action == "suppress" and not operator_note:
        raise HTTPException(
            status_code=400,
            detail="operator_note is required when suppressing an opportunity",
        )

    existing_plan_ids = [
        str(v)
        for v in (opportunity.get("linked_experiment_plan_ids") or [])
        if str(v).strip()
    ]
    existing_child_job_ids = [
        str(v) for v in (opportunity.get("child_job_ids") or []) if str(v).strip()
    ]
    ...
    async def _ensure_plan_ids(current_plan_ids: list[str]) -> list[str]:
        ...
        plan_id = await _create_plan_for_opportunity(
            ...
            note_ids=[
                str(v)
                for v in (opportunity.get("source_note_ids") or [])
                if str(v).strip()
            ]
            or [str(v) for v in (profile.latest_note_ids or []) if str(v).strip()],
            title=str(opportunity.get("title") or "Research opportunity"),
            hypothesis=str(
                opportunity.get("hypothesis") or opportunity.get("title") or ""
            ),
            ...
            generator_details={
                ...
                "supporting_evidence": opportunity.get("supporting_evidence")
                if isinstance(opportunity.get("supporting_evidence"), list)
                else [],
                "supporting_sources": opportunity.get("supporting_sources")
                if isinstance(opportunity.get("supporting_sources"), list)
                else [],
                "selected_hypothesis_ids": [opportunity["opportunity_id"]],
                "evidence_revision_at_launch": str(
                    opportunity.get("evidence_revision") or ""
                ).strip()
                or None,
    ...
    if action == "accept":
        ...
        materialization = await materialize_research_opportunity_experiment(
            ...
            benchmark_queries=[
                str(v) for v in (profile.benchmark_queries or []) if str(v).strip()
            ],
        ...
        child_job = await executor._create_domain_research_follow_up_job(
            ...
            if isinstance(profile.automation_policy, dict)
            else {},
            sandbox_profile_id=profile.sandbox_profile_id,
            profile_id=str(profile.id),
        )
        if child_job is None:
            raise HTTPException(
                status_code=400, detail="Failed to launch follow-up job"
            )
            ...
            dict.fromkeys(
                [
                    *(
                        [
                            str(v)
                            for v in (opportunity.get("child_job_ids") or [])
                            if str(v).strip()
                        ]
                    ),
                    str(child_job.id),
        ...
        follow_up_last_job_id = str(
            opportunity.get("follow_up_last_job_id") or ""
        ).strip()
        follow_up_outcome_status = (
            str(opportunity.get("follow_up_outcome_status") or "").strip().lower()
        )
        if follow_up_outcome_status not in {"failed", "cancelled"}:
            raise HTTPException(
                status_code=400,
                detail="Only failed or cancelled follow-ups can be relaunched",
            )
        if not follow_up_last_job_id:
            raise HTTPException(
                status_code=422,
                detail="Opportunity is missing its last follow-up job identifier",
            )
        try:
            ...
            raise HTTPException(
                status_code=422,
                detail="Opportunity has an invalid last follow-up job identifier",
            ) from exc
        inbox_item = (
            await db.execute(
        ...
        if inbox_item is None:
            raise HTTPException(
                status_code=422,
                detail="Could not resolve a relaunchable inbox follow-up for this opportunity",
            )
        ...
        refreshed_rows = list_normalized_research_opportunities(
            refreshed_summary.get("opportunities")
            if isinstance(refreshed_summary.get("opportunities"), list)
            else refreshed_summary.get("idea_candidates")
        )
    ...
    decision_event_type = {
        ...
        "launch_validation": "validation_requeued"
        if opportunity.get("linked_validation_run_ids")
    ...
    await record_autonomy_decision_event(
        ...
        reason_code=(
            ...
            if action == "relaunch_follow_up"
                ...
                opportunities[idx].get("last_decision_reason_code")
                or opportunities[idx].get("last_blocked_reason_code")
                or action
            ).strip()
            or None
        ),
        status=str(
            opportunities[idx].get("stage")
            or opportunities[idx].get("autonomy_state")
            or ""
        ).strip()
        or None,
        severity="high"
        if str(opportunities[idx].get("stage") or "").strip().lower() == "blocked"
        else "medium",
        actor_mode="operator",
