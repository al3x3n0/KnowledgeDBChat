...
def test_profile_to_response_includes_collaboration_summary():
    ...
    response = _profile_to_response(profile, current_user=current_user, user_lookup=user_lookup)

    assert response.visibility == 'shared'
    assert [str(user_id) for user_id in response.shared_with_user_ids] == [str(shared_user_id)]
