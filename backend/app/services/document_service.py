...
class DocumentService:
    ...
    async def get_document(
        ...
        result = await db.execute(
            ...
            .options(
                ...
                selectinload(Document.owner_persona),
                selectinload(Document.persona_detections).selectinload(
                    DocumentPersonaDetection.persona
                ),
            )
    ...
    async def upload_file(
        ...
        try:
            logger.info(
                f"Starting upload for file: {file.filename}, content_type: {file.content_type}, size: {file.size if hasattr(file, 'size') else 'unknown'}"
            )
            ...
            if owner_user:
                owner_display_name = (
                    owner_user.full_name or owner_user.username or owner_user.email
                )
            ...
            if owner_user:
                await persona_service.assign_document_owner(
                    db, document, user=owner_user, platform_scope="file-upload"
                )
                await db.commit()
                await db.refresh(document)
            ...
            try:
                ...
                if transcription_service and transcription_service.is_supported_format(
                    file_path_obj
                ):
                    # Video/audio file
                    ...
                    logger.info(
                        f"Extracting text from regular document: {file.filename}"
                    )
                    try:
                        (
                            text_content,
                            extraction_metadata,
                        ) = await self.text_processor.extract_text(
                        ...
                        logger.warning(
                            f"Failed to extract text from {file.filename}: {e}. Continuing with empty content."
                        )
                        document.content = ""

                    # Process document asynchronously
                    await self._process_document_async(
                        document, db, user_id=owner_user.id if owner_user else None
                    )

                await db.commit()
                await db.refresh(document)
                logger.info(
                    f"Successfully uploaded and processed document: {document.id}"
                )
            except Exception as e:
                logger.error(
                    f"Error during file processing for {file.filename}: {e}",
                    exc_info=True,
                )
    ...
    async def _save_uploaded_file(
        self, document_id: UUID, file: UploadFile, content: bytes
    ) -> str:
        """
        ...
        try:
            ...
            if await _get_flag("knowledge_graph_enabled"):
                try:
                    ...
                    from app.services.knowledge_extraction import (
                        extractor as rule_extractor,
                    )
                    from app.services.knowledge_extraction import llm_extractor
                    ...
                    if user_id is not None:
                        try:
                            ...
                            effective_user_settings = UserLLMSettings.from_preferences(
                                user_prefs
                            )
                        except Exception as _pref_err:
                            logger.warning(
                                f"Failed to load user LLM settings for KG ingestion: {_pref_err}"
                            )

                    use_llm = bool(
                        getattr(settings, "KG_LLM_EXTRACTION_ENABLED", False)
                    )
                    ...
                    await db.commit()
                    logger.info(
                        f"KG extraction for document {document.id}: mentions={total_mentions}, relations={total_relations}"
                    )
                except Exception as e:
                    logger.warning(
            ...
            if await _get_flag("summarization_enabled") and await _get_flag(
                ...
                try:
                    ...
                    logger.info(
                        f"Scheduled auto-summarization for document {document.id}"
                    )
                except Exception as _e:
                    logger.warning(
                        f"Failed to schedule auto-summarization for {document.id}: {_e}"
                    )
    ...
    async def delete_document(self, document_id: UUID, db: AsyncSession) -> bool:
        ...
        try:
            logger.info(
                f"Starting deletion of document {document_id}: {document.title}"
            )
            ...
            if document.file_path:
                try:
                    ...
                    if deleted:
                        logger.info(
                            f"Successfully deleted file from MinIO: {document.file_path}"
                        )
                    else:
                        logger.warning(
            ...
            try:
                logger.info(
                    f"Deleting chunks from vector store for document {document_id}"
                )
                await self.vector_store.delete_document_chunks(document_id)
                logger.info(
                    f"Successfully deleted chunks from vector store for document {document_id}"
                )
            except Exception as e:
                logger.warning(
            ...
            try:
                ...
                logger.info(
                    f"Deleted {deleted_chunks} chunks from database for document {document_id}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to delete chunks from database for document {document_id}: {e}"
                )
            ...
            try:
                ...
                async with db.begin_nested():
                    ...
                    if upload_sessions:
                        logger.info(
                            f"Found {len(upload_sessions)} upload session(s) referencing document {document_id}"
                        )
                ...
                if "upload_sessions" in locals() and upload_sessions:
                    logger.info(
                        f"Deleted {len(upload_sessions)} upload session(s) for document {document_id}"
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to delete upload sessions for document {document_id}: {e}"
                )
    ...
    async def delete_documents_by_source(
        self, source_id: UUID, db: AsyncSession
    ) -> int:
        ...
        for doc_id in document_ids:
            try:
                ...
                logger.warning(
                    f"Failed to delete document {doc_id} for source {source_id}: {exc}"
                )
        return deleted

    async def reprocess_document(
        self, document_id: UUID, db: AsyncSession, user_id: Optional[UUID] = None
    ) -> bool:
    ...
    async def summarize_document(
        ...
        heavy_threshold = getattr(
            _settings, "SUMMARIZATION_HEAVY_THRESHOLD_CHARS", 30000
        )
        prefer_deepseek = (
            bool(getattr(_settings, "DEEPSEEK_API_KEY", None))
            and len(text) > heavy_threshold
        )
        ...
        async def _generate_with_fallback(
            query: str, max_tokens: int, use_deepseek: bool = False
        ) -> str:
            ...
            try:
                ...
                if use_deepseek:
                    ...
                    logger.warning(
                        f"Remote model (DeepSeek) failed: {e}. Falling back to local model (Ollama)."
                    )
                    try:
                        result = await self.llm.generate_response(
                            query=query,
                        ...
                        actual_model_used = model or self.llm.default_model
                        logger.info(
                            "Successfully used local model (Ollama) after DeepSeek failure"
                        )
                        return result
        ...
        if len(chunks) == 1:
            ...
            summary = await _generate_with_fallback(
                query, max_tokens=800, use_deepseek=prefer_deepseek
            )
        else:
            ...
            for i, ch in enumerate(chunks, 1):
                q = f"{system}\n\nChunk {i}/{len(chunks)}:\n{ch}"
                s = await _generate_with_fallback(
                    q, max_tokens=600, use_deepseek=prefer_deepseek
                )
            ...
            summary = await _generate_with_fallback(
                combine_prompt, max_tokens=1000, use_deepseek=prefer_deepseek
            )
    ...
    async def create_document_source(
        self, name: str, source_type: str, config: Dict[str, Any], db: AsyncSession
    ) -> DocumentSource:
        """Create a new document source."""
        source = DocumentSource(name=name, source_type=source_type, config=config)
