...
class GitRepoSourceRequest(BaseModel):
    ...
    name: Optional[str] = Field(
        default=None, description="Optional display name for the source"
    )
...
class ArxivSourceRequest(BaseModel):
    ...
    name: Optional[str] = Field(
        default=None, description="Optional source name to display"
    )
    search_queries: Optional[List[str]] = Field(
        default=None, description="List of ArXiv API search_query expressions"
    )
    paper_ids: Optional[List[str]] = Field(
        default=None, description="Explicit ArXiv identifiers (e.g. 2401.12345)"
    )
    ...
    max_results: int = Field(
        default=50, ge=1, le=200, description="Max results per query"
    )
    start: int = Field(
        default=0, ge=0, le=1000, description="Offset for paginated queries"
    )
    ...
    auto_summarize: bool = Field(
        default=True, description="Queue summarization for ingested papers"
    )
    auto_literature_review: bool = Field(
        default=False,
        description="Generate a literature review document after ingestion",
    )
    auto_enrich_metadata: bool = Field(
        default=True,
        description="Enrich papers with BibTeX/DOI metadata after ingestion",
    )
    auto_extract_structure: bool = Field(
        default=False, description="Queue structured paper extraction after ingestion"
    )
    topic: Optional[str] = Field(
        default=None, description="Optional topic label for literature review/reporting"
    )
...
class IngestUrlRequest(BaseModel):
    ...
    url: str = Field(
        ..., description="URL to ingest (http/https)", min_length=1, max_length=2000
    )
    title: Optional[str] = Field(
        default=None, description="Optional title override (single-document mode)"
    )
    tags: Optional[List[str]] = Field(
        default=None, description="Optional tags to apply"
    )
    ...
    follow_links: bool = Field(
        default=False, description="Crawl a small link graph from the start page"
    )
    max_pages: int = Field(
        default=1, ge=1, le=25, description="Max pages to fetch when crawling"
    )
    max_depth: int = Field(default=0, ge=0, le=5, description="Max crawl depth")
    same_domain_only: bool = Field(
        default=True, description="Only follow links on the same domain"
    )
    one_document_per_page: bool = Field(
        default=False, description="Create/update one document per page URL"
    )
    ...
    max_content_chars: int = Field(
        ...
        ge=1000,
        le=500000,
        description="Max characters per page to ingest",
    )
...
class InstantArxivIngestRequest(BaseModel):
    ...
    auto_summarize: bool = Field(
        default=False, description="Queue background summarization after ingestion"
    )
    auto_enrich: bool = Field(
        default=False, description="Queue background metadata enrichment (BibTeX, DOI)"
    )
    auto_extract: bool = Field(
        default=False, description="Queue background structured paper extraction"
    )
...
class ResearchPresentationRequest(BaseModel):
    ...
    slide_count: int = Field(
        default=10, ge=5, le=25, description="Number of slides to generate"
    )
    include_arxiv: bool = Field(
        default=True, description="Search arXiv for relevant papers to include"
    )
    arxiv_max_papers: int = Field(
        default=5, ge=0, le=15, description="Maximum arXiv papers to incorporate"
    )
    style: str = Field(
        default="technical",
    ...
    include_diagrams: bool = Field(
        default=True, description="Include auto-generated diagrams"
    )
    source_document_ids: Optional[List[str]] = Field(
        default=None, description="Specific document IDs to include as sources"
    )
