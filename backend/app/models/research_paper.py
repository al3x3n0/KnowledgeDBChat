...
class ResearchPaper(Base):
    ...
    claims = relationship("PaperClaim", back_populates="paper", cascade="all, delete-orphan")
    extraction_jobs = relationship("PaperExtractionJob", back_populates="paper", cascade="all, delete-orphan")
...
class PaperClaim(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    paper_id = Column(UUID(as_uuid=True), ForeignKey("research_papers.id", ondelete="CASCADE"), nullable=False, index=True)
...
class PaperExtractionJob(Base):
    ...
    document_id = Column(UUID(as_uuid=True), ForeignKey("documents.id", ondelete="CASCADE"), nullable=False, index=True)
    source_id = Column(UUID(as_uuid=True), ForeignKey("document_sources.id", ondelete="SET NULL"), nullable=True, index=True)
    paper_id = Column(UUID(as_uuid=True), ForeignKey("research_papers.id", ondelete="SET NULL"), nullable=True, index=True)
