...
class BenchmarkSuite(Base):
    ...
    benchmark_family = Column(
        String(64), nullable=False, default="compiler_regression", index=True
    )
    ...
    baselines = relationship(
        "BenchmarkBaseline", back_populates="suite", cascade="all, delete-orphan"
    )

    def to_dict(
        self, *, include_cases: bool = True, include_baselines: bool = True
    ) -> dict[str, Any]:
        return {
            "id": self.id,
            ...
            "cases": [
                case.to_dict()
                for case in sorted(
                    self.cases, key=lambda row: (int(row.rank or 0), row.name)
                )
            ]
            if include_cases
            else [],
...
class BenchmarkBaseline(Base):
    ...
    case_id = Column(
        String(120),
        ForeignKey("benchmark_cases.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    name = Column(String(200), nullable=False)
