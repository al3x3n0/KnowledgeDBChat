...
def _normalize_collaboration(payload: Any, *, fallback_owner_user_id: Optional[str] = None) -> dict[str, Any]:
    ...
    owner_user_id = str(raw.get("owner_user_id") or fallback_owner_user_id or "").strip() or None
...
def _is_backlog_visible_to_user(item: CodingBacklogItem, user_id: UUID) -> bool:
    ...
    if _normalize_visibility(getattr(item, "visibility", "private")) != "shared":
        return False
    return str(user_id) in _normalize_uuid_list(getattr(item, "shared_with_user_ids", None), 200)


async def _get_visible_backlog_item_or_404(db: AsyncSession, item_id: UUID, user_id: UUID) -> CodingBacklogItem:
...
def _recompute_portfolio_progress(decomposition: dict[str, Any]) -> dict[str, Any]:
    planned = decomposition.get("planned_slices") if isinstance(decomposition.get("planned_slices"), list) else []
    completed = [str(v).strip() for v in (decomposition.get("completed_slices") if isinstance(decomposition.get("completed_slices"), list) else []) if str(v).strip()]
    failed = [str(v).strip() for v in (decomposition.get("failed_slices") if isinstance(decomposition.get("failed_slices"), list) else []) if str(v).strip()]
    return {
        "total_slices": len(planned),
        "pending_slices": sum(1 for row in planned if str((row or {}).get("status") or "").strip().lower() in {"pending", "repairing", "retrying", "applying", "deferred"}),
        "completed_slices": len(completed),
        "failed_slices": len(failed),
        "auto_applied_slices": sum(1 for row in planned if str((row or {}).get("promotion_decision") or "").strip().lower() == "auto_applied"),
        "proposal_only_slices": sum(1 for row in planned if str((row or {}).get("promotion_decision") or "").strip().lower() in {"proposal_only", "patch_pr"}),
    }
...
def _normalize_decomposition(item: CodingBacklogItem) -> dict[str, Any]:
    ...
    if isinstance(raw, dict):
        dec.update({k: deepcopy(v) for k, v in raw.items() if k in dec or k == "planned_slices"})
    ...
    dec["lineage_summary"] = {
        "repair_job_count": sum(len((row.get("job_lineage") or {}).get("repair_job_ids") or []) for row in dec.get("planned_slices") or [] if isinstance(row, dict)),
        "apply_job_count": sum(len((row.get("job_lineage") or {}).get("apply_job_ids") or []) for row in dec.get("planned_slices") or [] if isinstance(row, dict)),
        "patch_pr_count": sum(len((row.get("job_lineage") or {}).get("patch_pr_ids") or []) for row in dec.get("planned_slices") or [] if isinstance(row, dict)),
        "proposal_count": sum(len((row.get("job_lineage") or {}).get("proposal_ids") or []) for row in dec.get("planned_slices") or [] if isinstance(row, dict)),
        "operator_action_count": sum(len(row.get("manual_promotion_history") or []) for row in dec.get("planned_slices") or [] if isinstance(row, dict)),
    }
    return dec
...
def _append_backlog_timeline(decomposition: dict[str, Any], entry: dict[str, Any]) -> None:
    rows = decomposition.get("backlog_timeline") if isinstance(decomposition.get("backlog_timeline"), list) else []
    rows.append(entry)
...
def _append_manual_promotion_history(slice_state: dict[str, Any], *, action: str, operator_note: Optional[str] = None, proposal_id: Optional[str] = None, patch_pr_id: Optional[str] = None, apply_job_id: Optional[str] = None) -> None:
    rows = slice_state.get("manual_promotion_history") if isinstance(slice_state.get("manual_promotion_history"), list) else []
    rows.append(
        {
            "at": datetime.utcnow().isoformat(),
            "action": action,
...
def _find_slice(decomposition: dict[str, Any], slice_id: Optional[str]) -> Optional[dict[str, Any]]:
...
def _append_unique(values: Any, value: Optional[str]) -> list[str]:
    out = [str(v).strip() for v in values if str(v).strip()] if isinstance(values, list) else []
...
def _upsert_promotion_decision(decomposition: dict[str, Any], entry: dict[str, Any]) -> None:
    rows = decomposition.get("promotion_decisions") if isinstance(decomposition.get("promotion_decisions"), list) else []
    slice_id = str(entry.get("slice_id") or "").strip()
    kept = [row for row in rows if str((row or {}).get("slice_id") or "").strip() != slice_id]
    kept.append(entry)
...
def _allowed_actions_for_slice(slice_state: dict[str, Any]) -> list[str]:
    ...
    if status in {"proposal_only", "blocked", "patch_pr"} or bool(slice_state.get("awaiting_operator_action")):
        return ["apply_override", "create_patch_pr", "keep_proposal_only", "relaunch_slice", "skip_slice"]
    if status == "failed":
...
def _recommended_action_for_slice(slice_state: dict[str, Any]) -> Optional[str]:
    ...
    if blocked_reason in {"blocked_path_prefix", "max_files_touched_exceeded", "require_patch_pr"}:
...
def _refresh_waiting_metadata(decomposition: dict[str, Any], slice_state: Optional[dict[str, Any]]) -> None:
    for row in decomposition.get("planned_slices") or []:
        row["allowed_slice_actions"] = _allowed_actions_for_slice(row) if row is slice_state else []
    if slice_state is not None:
        ...
        slice_state["recommended_next_action"] = _recommended_action_for_slice(slice_state)
...
def _set_latest_summary(item: CodingBacklogItem, decomposition: dict[str, Any], *, status_value: str, slice_state: Optional[dict[str, Any]] = None, note: Optional[str] = None, extra: Optional[dict[str, Any]] = None) -> None:
    summary = {
        "status": status_value,
        "portfolio_progress": _recompute_portfolio_progress(decomposition),
        "waiting_on_operator_action": bool(slice_state and slice_state.get("awaiting_operator_action")),
        "allowed_slice_actions": (slice_state.get("allowed_slice_actions") if slice_state else []) or [],
        "recommended_next_action": slice_state.get("recommended_next_action") if slice_state else None,
        "active_slice_id": slice_state.get("slice_id") if slice_state else decomposition.get("active_slice_id"),
...
def _build_why_not_repair_summary(item: CodingBacklogItem) -> Optional[dict[str, Any]]:
    ...
    return {
        "review_reason": str(lineage.get("originating_swarm_review_reason") or "").strip() or None,
        "route_mode": str(lineage.get("originating_swarm_route_mode") or "").strip() or None,
        "candidate_role": str(lineage.get("originating_swarm_candidate_role") or "").strip() or None,
        "recommended_next_action": str(summary.get("recommended_next_action") or "").strip() or None,
...
def _derive_operator_queue_state(item: CodingBacklogItem) -> str:
    ...
    if str(item.status or "").strip().lower() in {"cancelled"} and closure_reason:
        return "superseded" if closure_reason in {"duplicate", "outdated"} else "blocked"
    ...
    is_auto_routed = str(lineage.get("originating_swarm_route_mode") or "").strip().lower() == "auto"
...
def _to_response(item: CodingBacklogItem, *, current_user: Optional[User] = None, user_lookup: Optional[dict[str, User]] = None) -> CodingBacklogItemResponse:
    collaboration = _normalize_collaboration(getattr(item, "collaboration", None), fallback_owner_user_id=str(item.user_id))
    latest_summary = item.latest_summary if isinstance(item.latest_summary, dict) else {}
    return CodingBacklogItemResponse.model_validate(
        {
            **item.__dict__,
            "child_job_ids": [str(v).strip() for v in _normalize_str_list(item.child_job_ids)],
            "visibility": _normalize_visibility(getattr(item, "visibility", "private")),
            "shared_with_user_ids": _normalize_uuid_list(getattr(item, "shared_with_user_ids", None), 200),
            "collaboration": collaboration,
            "collaboration_summary": build_collaboration_summary(
                owner_user_id=str(collaboration.get("owner_user_id") or item.user_id),
                visibility=str(collaboration.get("visibility") or getattr(item, "visibility", "private")),
                shared_with_user_ids=list(collaboration.get("shared_with_user_ids") or getattr(item, "shared_with_user_ids", None) or []),
                assigned_user_id=str(collaboration.get("assigned_user_id") or getattr(item, "assigned_user_id", "")).strip() or None,
                assigned_by_user_id=str(collaboration.get("assigned_by_user_id") or getattr(item, "assigned_by_user_id", "")).strip() or None,
                assigned_at=str(collaboration.get("assigned_at") or getattr(item, "assigned_at", "") or "").strip() or None,
                note=str(collaboration.get("note") or "").strip() or None,
                current_user_id=str(current_user.id) if current_user is not None else None,
                user_lookup=user_lookup,
            ),
            "operator_queue_state": _derive_operator_queue_state(item),
            "closure_reason": _normalize_closure_reason(latest_summary.get("closure_reason")),
            "why_not_repair": _build_why_not_repair_summary(item),
        }
    )


async def _build_backlog_user_lookup(db: AsyncSession, *, current_user: User) -> dict[str, User]:
...
def _build_orchestrator_chain_config(backlog_item_id: UUID, previous_child_kind: str) -> dict[str, Any]:
    return {
...
def _attach_terminal_continuation(chain_config: Optional[dict], backlog_item_id: UUID, previous_child_kind: str) -> Optional[dict]:
    ...
    while isinstance(cursor.get("child_jobs"), list) and cursor.get("child_jobs"):
        ...
        if not isinstance(child.get("chain_config"), dict):
            child["chain_config"] = _build_orchestrator_chain_config(backlog_item_id, previous_child_kind)
            return updated
...
async def _create_orchestrator_job(
    ...
    *,
    ...
    job = AgentJob(
        ...
        job_type="analysis",
        goal=str(item.portfolio_goal or "").strip()[:8000] or "Orchestrate coding backlog execution",
        config={
            "deterministic_runner": "coding_backlog_orchestrator",
...
async def _spawn_slice_repair_job(
    ...
    *,
    ...
    if not template:
        raise HTTPException(status_code=500, detail="Repo bug triage template unavailable")
    ...
    search_query = str(slice_state.get("search_query") or "").strip() or " ".join(
        part for part in [("" if scope == "auto" else scope), symptom, " ".join(file_paths[:2])] if part
    ).strip()[:500]
    ...
    if operator_note:
        merged_config["coding_backlog_operator_note"] = str(operator_note).strip()[:5000]
    ...
    if operator_note:
        _append_manual_promotion_history(slice_state, action="relaunch_slice" if prev_status else "repair_job_started", operator_note=operator_note)
    ...
    *,
    ...
    apply_job = AgentJob(
        ...
        config={
            ...
            "coding_backlog_slice_id": str(slice_state.get("slice_id") or ""),
            "coding_backlog_operator_note": str(operator_note or "").strip()[:5000] or None,
        },
        user_id=item.user_id,
        status=AgentJobStatus.PENDING.value,
        parent_job_id=item.orchestrator_job_id,
    ...
    _append_manual_promotion_history(slice_state, action="apply_override", operator_note=operator_note, proposal_id=proposal_id, apply_job_id=str(apply_job.id))
...
async def list_coding_backlog_items(
    ...
    if status_filter:
        stmt = stmt.where(CodingBacklogItem.status == str(status_filter).strip().lower())
    rows = [row for row in list((await db.execute(stmt.order_by(desc(CodingBacklogItem.updated_at)))).scalars().all()) if _is_backlog_visible_to_user(row, current_user.id)]
    if visibility_scope == "mine":
        rows = [row for row in rows if str(row.user_id) == str(current_user.id)]
    elif visibility_scope == "shared":
        rows = [row for row in rows if str(row.user_id) != str(current_user.id)]
    if assigned_user_id:
        rows = [row for row in rows if str(getattr(row, "assigned_user_id", "") or "") == str(assigned_user_id)]
    total = len(rows)
    rows = rows[offset:offset + limit]
    user_lookup = await _build_backlog_user_lookup(db, current_user=current_user)
    return CodingBacklogItemListResponse(
        items=[_to_response(item, current_user=current_user, user_lookup=user_lookup) for item in rows],
        total=total,
        limit=limit,
        offset=offset,
    )
...
@router.post("", response_model=CodingBacklogItemResponse, status_code=status.HTTP_201_CREATED)
async def create_coding_backlog_item(
    ...
    item = CodingBacklogItem(
        ...
        shared_with_user_ids=list(collaboration_payload.get("shared_with_user_ids") or []) or None,
        assigned_user_id=UUID(str(collaboration_payload.get("assigned_user_id"))) if str(collaboration_payload.get("assigned_user_id") or "").strip() else None,
        assigned_by_user_id=UUID(str(collaboration_payload.get("assigned_by_user_id"))) if str(collaboration_payload.get("assigned_by_user_id") or "").strip() else None,
        assigned_at=datetime.fromisoformat(str(collaboration_payload.get("assigned_at"))) if str(collaboration_payload.get("assigned_at") or "").strip() else payload.assigned_at,
        collaboration=collaboration_payload,
        policy=_normalize_policy(payload.policy),
        lineage=deepcopy(payload.lineage) if isinstance(payload.lineage, dict) else None,
...
async def update_coding_backlog_item(
    ...
    if str(item.user_id) != str(current_user.id):
        raise HTTPException(status_code=403, detail="Only the backlog owner can edit this item")
    collaboration_source = item.collaboration if isinstance(item.collaboration, dict) else {}
    ...
    if payload.shared_with_user_ids is not None:
        item.shared_with_user_ids = _normalize_uuid_list(payload.shared_with_user_ids, 200) or None
    ...
    if payload.assigned_user_id is not None:
        ...
        collaboration_source["assigned_user_id"] = str(payload.assigned_user_id) if payload.assigned_user_id else None
    if payload.assigned_by_user_id is not None:
        item.assigned_by_user_id = payload.assigned_by_user_id
        collaboration_source["assigned_by_user_id"] = str(payload.assigned_by_user_id) if payload.assigned_by_user_id else None
    ...
    if payload.visibility is not None or payload.shared_with_user_ids is not None or payload.assigned_user_id is not None or payload.assigned_by_user_id is not None or payload.assigned_at is not None or payload.collaboration is not None:
        item.collaboration = _normalize_collaboration(collaboration_source, fallback_owner_user_id=str(item.user_id))
    if payload.policy is not None:
        item.policy = _normalize_policy(payload.policy)
    if payload.lineage is not None:
        item.lineage = deepcopy(payload.lineage) if isinstance(payload.lineage, dict) else None
...
async def act_on_coding_backlog_item(
    ...
    collaboration = _normalize_collaboration(item.collaboration, fallback_owner_user_id=str(item.user_id))

    owner_only_actions = {"cancel", "apply_override", "create_patch_pr", "keep_proposal_only", "relaunch_slice", "skip_slice", "close"}
    collaborator_actions = {"assign_backlog", "clear_backlog_assignment", "update_backlog_note", "start", "resume", "pause"}
    if action in owner_only_actions and str(item.user_id) != str(current_user.id):
        raise HTTPException(status_code=403, detail="Only the backlog owner can perform this action")
    if action in collaborator_actions and not _is_backlog_visible_to_user(item, current_user.id):
        raise HTTPException(status_code=403, detail="Backlog item not visible")

    if action in {"apply_override", "create_patch_pr", "keep_proposal_only", "relaunch_slice", "skip_slice"} and slice_state is None:
        raise HTTPException(status_code=400, detail="slice_id is required for this action")

    if slice_state is not None and not slice_state.get("allowed_slice_actions") and not bool(slice_state.get("awaiting_operator_action")) and action not in {"relaunch_slice", "skip_slice"}:
        raise HTTPException(status_code=409, detail="Slice is not awaiting operator action")
    ...
    if action == "assign_backlog":
        ...
        collaboration = _normalize_collaboration(
            {
                **collaboration,
                "visibility": "shared" if bool(collaboration.get("shared_with_user_ids")) or str(item.user_id) != assigned_user_id else collaboration.get("visibility"),
                "assigned_user_id": assigned_user_id,
        ...
        item.visibility = str(collaboration.get("visibility") or item.visibility or "private")
        item.shared_with_user_ids = list(collaboration.get("shared_with_user_ids") or []) or None
        item.collaboration = collaboration
        _append_backlog_timeline(
            decomposition,
            _timeline_entry(actor="user", action="assign_backlog", note=operator_note, metadata={"assigned_user_id": assigned_user_id}),
        )
        item.decomposition = decomposition
        ...
        _append_backlog_timeline(
            decomposition,
            _timeline_entry(actor="user", action="clear_backlog_assignment", note=operator_note),
        )
        item.decomposition = decomposition
        ...
        _append_backlog_timeline(
            decomposition,
            _timeline_entry(actor="user", action="update_backlog_note", note=operator_note),
        )
        item.decomposition = decomposition
        ...
        item.status = "paused"
        item.updated_at = datetime.utcnow()
        _append_backlog_timeline(decomposition, _timeline_entry(actor="user", action="pause", previous_status=previous_status, new_status="paused", note=operator_note))
        item.decomposition = decomposition
    elif action == "cancel":
        if not closure_reason:
            raise HTTPException(status_code=400, detail="closure_reason is required for cancel")
        previous_status = str(item.status or "").strip() or None
        ...
        if not closure_reason:
            raise HTTPException(status_code=400, detail="closure_reason is required for close")
        previous_status = str(item.status or "").strip() or None
        item.status = "completed" if closure_reason in {"fixed_through_backlog", "promoted_to_repair"} else "cancelled"
        item.completed_at = datetime.utcnow()
        item.updated_at = datetime.utcnow()
        _append_backlog_timeline(
            decomposition,
        ...
        proposal_id = str(slice_state.get("selected_proposal_id") or item.latest_proposal_id or "").strip()
        proposal_uuid = None
        try:
            proposal_uuid = UUID(proposal_id)
        except Exception:
            raise HTTPException(status_code=400, detail="Slice does not have a valid proposal to apply")
        proposal = await db.get(CodePatchProposal, proposal_uuid)
        if not proposal or proposal.user_id != current_user.id:
            raise HTTPException(status_code=404, detail="Proposal not found")
        apply_job = await _spawn_slice_apply_job(item, slice_state, db=db, proposal_id=proposal_id, operator_note=operator_note)
        ...
        decomposition["portfolio_progress"] = _recompute_portfolio_progress(decomposition)
        item.decomposition = decomposition
        _set_latest_summary(
            item,
            decomposition,
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="Slice does not have a valid proposal to promote")
        proposal = await db.get(CodePatchProposal, proposal_uuid)
        if not proposal or proposal.user_id != current_user.id:
            raise HTTPException(status_code=404, detail="Proposal not found")
        pr = PatchPR(
        ...
        slice_state["operator_note"] = operator_note
        slice_state["operator_acted_at"] = datetime.utcnow().isoformat()
        slice_state["completed_at"] = slice_state.get("completed_at") or datetime.utcnow().isoformat()
        _clear_waiting_metadata(slice_state)
        _append_slice_timeline(
            slice_state,
            _timeline_entry(
        ...
        _append_artifact_history(slice_state, "proposal", proposal_id, "Selected proposal")
        _append_manual_promotion_history(slice_state, action="create_patch_pr", operator_note=operator_note, proposal_id=proposal_id, patch_pr_id=str(pr.id))
        _append_backlog_timeline(
            decomposition,
            _timeline_entry(
                actor="user",
        ...
        decomposition["completed_slices"] = _append_unique(decomposition.get("completed_slices"), slice_state.get("slice_id"))
        _upsert_promotion_decision(
            decomposition,
            {
                ...
                "decision": "patch_pr",
                "proposal_id": proposal_id,
                "blocked_reason": str(slice_state.get("blocked_reason") or "").strip() or None,
                "proposal_confidence": float(slice_state.get("proposal_confidence", 0.0) or 0.0),
        ...
        proposal_id = str(slice_state.get("selected_proposal_id") or item.latest_proposal_id or "").strip() or None
        previous_status = str(slice_state.get("status") or "").strip() or None
        ...
        _append_lineage_id(slice_state, "proposal_ids", proposal_id)
        _append_artifact_history(slice_state, "proposal", proposal_id, "Selected proposal")
        _append_manual_promotion_history(slice_state, action="keep_proposal_only", operator_note=operator_note, proposal_id=proposal_id)
        _append_backlog_timeline(
            decomposition,
            _timeline_entry(
                actor="user",
        ...
        repair_job = await _spawn_slice_repair_job(item, slice_state, db=db, operator_note=operator_note)
        ...
        _append_manual_promotion_history(slice_state, action="skip_slice", operator_note=operator_note)
        _append_backlog_timeline(
            decomposition,
            _timeline_entry(
                actor="user",
        ...
        if next_slice is not None:
            repair_job = await _spawn_slice_repair_job(item, next_slice, db=db, operator_note=operator_note)
            item.child_job_ids = _append_unique(item.child_job_ids, str(repair_job.id))
            item.current_job_id = repair_job.id
            item.status = "running"
            ...
            _set_latest_summary(
                ...
                extra={"current_child_job_id": str(repair_job.id), "waiting_on_operator_action": False},
            )
        else:
            item.decomposition = decomposition
            item.status = "completed"
