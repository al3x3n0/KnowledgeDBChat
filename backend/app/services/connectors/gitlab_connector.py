...
class GitLabConnector(BaseConnector):
            ...
            try:
                ...
                if project.get("include_files", True):
                    ...
                    if self.use_gitignore:
                        try:
                            await self._merge_gitignore(
                                project_info["id"], project_info.get("default_branch")
                            )
                            await self._merge_nested_gitignores(
                                project_info["id"], project_info.get("default_branch")
                            )
                        except Exception:
                            pass
                ...
                # Get merge requests
                if self.include_merge_requests and project.get(
            ...
            for project in self.projects:
                ...
                try:
                    ...
                    for file_path in files:
                        ...
                        if (
                            self.file_extensions
                            and file_ext not in self.file_extensions
                        ):
                            continue
                ...
                if self.include_issues and project.get("include_issues", True):
                    changed.extend(
                        await self._list_issues(project_id, updated_after=since)
                    )
                ...
                if self.include_merge_requests and project.get(
                    ...
                    changed.extend(
                        await self._list_merge_requests(project_id, updated_after=since)
                    )
        except Exception as e:
    ...
    async def _get_repository_files(self, project_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error getting GitLab repository files for project {project_id}: {e}"
            )
    ...
    async def _merge_nested_gitignores(
        self, project_id: int, default_branch: Optional[str] = None
    ) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Failed nested .gitignore merge for project {project_id}: {e}"
            )
    ...
    async def _get_wiki_pages(self, project_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error getting GitLab wiki pages for project {project_id}: {e}"
            )
    ...
    async def _list_issues(
        self, project_id: int, updated_after: Optional[datetime] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.client.get(
                f"{self.base_url}/api/v4/projects/{project_id}/issues", params=params
            )
            if response.status_code != 200:
                return issues
            issue_list = response.json()
    ...
    async def _get_merge_requests(self, project_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error getting GitLab merge requests for project {project_id}: {e}"
            )
    ...
    async def _list_changed_files(self, project_id: int, since: datetime) -> List[str]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to list changed files for project {project_id}: {e}"
            )
        return paths
    ...
    async def _get_wiki_metadata(
        self, project_id: str, wiki_slug: str
    ) -> Dict[str, Any]:
        """Get metadata for a wiki page."""
        return {"project_id": project_id, "wiki_slug": wiki_slug, "type": "wiki_page"}

    async def _get_issue_metadata(
        self, project_id: str, issue_iid: str
    ) -> Dict[str, Any]:
        """Get metadata for an issue."""
        return {"project_id": project_id, "issue_iid": issue_iid, "type": "issue"}

    async def _get_merge_request_metadata(
    ...
    async def list_branches(self, project_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            while True:
                ...
                if resp.status_code != 200:
                    logger.warning(
                        f"GitLab branch list failed for {project_id}: {resp.status_code} {resp.text}"
                    )
                    break
                data = resp.json()
                if not data:
            ...
            logger.error(
                f"Error listing branches for GitLab project {project_id}: {exc}"
            )
        return branches

    async def compare_branches(
        self, project_id: str, base_branch: str, compare_branch: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if resp.status_code != 200:
                raise ValueError(
                    f"GitLab compare API failed: {resp.status_code} {resp.text}"
                )
            return resp.json()
        except Exception as exc:
    ...
    async def get_repository_info(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            resp = await self.client.get(
                f"{self.base_url}/api/v4/projects/{encoded_id}"
            )
            if resp.status_code != 200:
                logger.error(
                    f"Failed to get repo info for {project_id}: {resp.status_code}"
                )
                return {}
            data = resp.json()
            return {
    ...
    async def get_recent_commits(
        self, project_id: str, limit: int = 20, branch: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    f"Failed to get commits for {project_id}: {resp.status_code}"
                )
                return commits
    ...
    async def get_contributors(
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    f"Failed to get contributors for {project_id}: {resp.status_code}"
                )
                return contributors
    ...
    async def get_languages(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            resp = await self.client.get(
                f"{self.base_url}/api/v4/projects/{encoded_id}/languages"
            )
            if resp.status_code != 200:
                logger.warning(
                    f"Failed to get languages for {project_id}: {resp.status_code}"
                )
    ...
    async def get_file_tree(
        self, project_id: str, max_depth: int = 3
    ) -> Dict[str, Any]:
    ...
    async def get_open_issues(
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    f"Failed to get issues for {project_id}: {resp.status_code}"
                )
                return issues
    ...
    async def get_open_merge_requests(
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    f"Failed to get MRs for {project_id}: {resp.status_code}"
                )
                return mrs
                ...
                mrs.append(
                    {
                        ...
                        "url": item.get("web_url"),
                        "draft": item.get("work_in_progress", False)
                        or item.get("draft", False),
                    }
                )
        except Exception as e:
