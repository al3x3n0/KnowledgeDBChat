...
**This is now fixed.** The image carries `g++` and `libstdc++-12-dev`, and the
tool's own probe reports `supported: true`, so the refusal lifted with no code
change — it asks the image by compiling a trivial static C++ program there,
rather than asserting. Verified end to end: a C++ workload using `std::vector`
and `std::string` was counter-sampled successfully, 9 intervals and 60 varying
counters, with the program's output (`cpp-corpus s=84`) proving libstdc++ was
genuinely linked and executing under gem5.

**And the corpus has now been run.** Godot 4.4.1's own `core/math` --
`Vector3::normalized()` and `AABB::expand_to()`, both `_FORCE_INLINE_`, so the
simulated code is the engine's -- staged through the tool as a 22-header
closure, 401 intervals, 60 varying counters. Details and the trace are in
`godot/`.

Two blockers sat behind the missing `g++`, neither recorded before: Godot's
`platform_config.h` is generated by SCons and absent from the tree (the fix is
to add `platform/linuxbsd` to the include path, where a usable one ships), and
`AABB::get_volume()` is not inline, so calling it turns a header-only corpus
into a build of the engine.

**The result is negative, and the first reading of it was a false positive.**
Cycles read entropy 1.563, persistence 1.387, best-beyond 0.146 against a null
of 0.035 -- surviving at 4.2x, with `branchPred.squashes_0::DirectCond` on top,
which is mechanistically exactly what six data-dependent comparisons inside
`expand_to` should produce. It is an artifact: cycles per interval run 18,445
to 18,646 between the 5th and 95th percentiles, a spread of **1.08%**. Three
quantile bins split that jitter into levels and the estimator found real
structure in what the quantiser invented.

So Godot's inline vector math has nothing worth predicting at this granularity,
and that is the answer rather than a failed run. `measure_predictability` now
reports the target's relative spread with every estimate and says so in the
verdict.
