...
class ConfluenceConnector(BaseConnector):
    ...
    async def get_document_metadata(self, identifier: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error getting Confluence document metadata {identifier}: {e}"
            )
            return {}
    ...
    async def _get_accessible_spaces(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = await self.client.get(
                f"{self.base_url}/rest/api/space", params={"limit": 100}
            )
    ...
    async def _get_space_attachments(self, space_key: str) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                attachments.append(
                    {
                        ...
                        "last_modified": last_modified,
                        "author": attachment.get("version", {})
                        .get("by", {})
                        .get("displayName"),
                        "file_type": attachment.get("extensions", {}).get(
                            "fileSize", "attachment"
                        ),
                        "metadata": {
                            ...
                            "container_title": attachment.get("container", {}).get(
                                "title", ""
                            ),
                            "media_type": media_type,
                            "file_size": attachment.get("extensions", {}).get(
                                "fileSize", 0
                            ),
                            "type": "attachment",
                        },
            ...
            logger.error(
                f"Error getting Confluence attachments for space {space_key}: {e}"
            )
        ...
        try:
            ...
            if download_response.status_code == 200:
                ...
                if media_type.startswith("text/"):
                    ...
                    return (
                        f"Attachment: {attachment_data.get('title', 'Unknown')}\n"
                        f"Type: {media_type}\n"
            ...
            logger.error(
                f"Error getting Confluence attachment content {attachment_id}: {e}"
            )
            return ""
    ...
    async def _get_comment_content(self, comment_id: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                author = (
                    comment_data.get("version", {})
                    .get("by", {})
                    .get("displayName", "Unknown")
                )
                container_title = comment_data.get("container", {}).get(
                    "title", "Unknown Page"
                )
