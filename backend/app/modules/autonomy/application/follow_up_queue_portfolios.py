"""Approve or reject queued research-portfolio opportunity follow-ups."""
...
class PortfolioFollowUpActionDependencies:
...
async def perform_portfolio_follow_up_queue_action(
    *,
    ...
    db: AsyncSession,
    current_user: User,
    deps: PortfolioFollowUpActionDependencies,
    ...
    if not normalized_opportunity_id:
        raise FollowUpQueueActionError(
            status_code=400,
            detail="portfolio_opportunity_id is required",
        )
    ...
    if opportunity_index < 0:
        raise FollowUpQueueActionError(
            status_code=404,
            detail="Portfolio opportunity not found",
        )
    ...
    if not review or review.get("review_type") != "follow_up_recommendation":
        raise FollowUpQueueActionError(
            status_code=400,
            detail=(
                "Portfolio opportunity is not currently waiting for "
                "follow-up approval"
            ),
        )
    ...
    if str(action or "").strip().lower() == "reject_launch":
        ...
        return AgentCheckpointQueueFollowUpActionResponse(
            ...
            portfolio_opportunity_id=normalized_opportunity_id,
    ...
    parent_job = await deps.resolve_parent_job(db=db, portfolio=portfolio)
    ...
    child_job = await executor._create_domain_research_follow_up_job(
        ...
        repo_source_ids=[
            str(value)
            for value in (opportunity.get("source_repo_ids") or [])
            if str(value).strip()
        ],
        benchmark_queries=[],
    ...
    deps.execute_agent_job_task.delay(str(child_job.id), str(portfolio.user_id))
    return AgentCheckpointQueueFollowUpActionResponse(
        portfolio_id=portfolio.id,
        portfolio_opportunity_id=normalized_opportunity_id,
