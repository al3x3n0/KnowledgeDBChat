...
class DockerToolConfig(BaseModel):
    ...
    # Container image
    image: str = Field(
        ..., description="Docker image name (e.g., 'python:3.11-slim', 'node:18')"
    )
    ...
    entrypoint: Optional[List[str]] = Field(
        None, description="Override the container entrypoint"
    )

    # Input/Output modes
    input_mode: Literal["stdin", "file", "both"] = Field(
        "stdin", description="How input is provided: stdin, file, or both"
    )
    output_mode: Literal["stdout", "file", "both"] = Field(
        "stdout", description="How output is captured: stdout, file, or both"
    )
    ...
    memory_limit: str = Field(
        "512m", description="Memory limit (e.g., '256m', '1g', '2g')"
    )
    cpu_limit: float = Field(
        1.0, ge=0.1, le=8.0, description="CPU limit (e.g., 0.5, 1.0, 2.0)"
    )
    ...
    working_dir: str = Field(
        "/workspace", description="Working directory inside the container"
    )

    # Network access
    network_enabled: bool = Field(
        False, description="Whether to enable network access (security consideration)"
    )
...
class DockerToolExecutionInput(BaseModel):
    ...
    stdin_data: Optional[str] = Field(
        None, description="Data to send to container via stdin"
    )
    input_content: Optional[str] = Field(
        None, description="Content to write to input file"
    )
    document_ids: Optional[List[str]] = Field(
        None, description="Document IDs to download and mount in the container"
    )
    environment_overrides: Optional[Dict[str, str]] = Field(
        None, description="Additional environment variables to set"
    )
