...
class DeterministicRunnerRegistry:
    ...
    def __init__(
        self, runners: Optional[Iterable[DeterministicJobRunner]] = None
    ) -> None:
    ...
    async def try_execute(
        ...
        *,
        ...
        if runner is None:
            return False, None
        return True, await runner.run(
            job=job, db=db, progress_callback=progress_callback
        )


def _resolve_handler(
    executor: Any, service_attr: str, service_method: str, fallback_method: str
):
    ...
    if callable(method):
        ...
        async def _handler(
            *, job: Any, db: Any, progress_callback: Optional[Callable[..., Any]] = None
        ) -> Dict[str, Any]:
            return await method(
                executor, job=job, db=db, progress_callback=progress_callback
            )
...
def _build_research_orchestration_runners(
    executor: Any,
) -> list[DeterministicJobRunner]:
    return [
        FunctionDeterministicJobRunner(
            "ai_hub_scientist",
            _resolve_handler(
                executor,
                "research_runner_service",
                "run_ai_hub_scientist",
                "_run_ai_hub_scientist",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "research_runner_service",
                "run_research_inbox_monitor",
                "_run_research_inbox_monitor",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "research_runner_service",
                "run_research_engineer_scientist",
                "_run_research_engineer_scientist",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "research_runner_service",
                "run_domain_research_orchestrator",
                "_run_domain_research_orchestrator",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "research_runner_service",
                "run_research_fleet_orchestrator",
                "_run_research_fleet_orchestrator",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "research_runner_service",
                "run_research_engineer_paper_update",
                "_run_research_engineer_paper_update",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "research_runner_service",
                "run_swarm_fan_in_aggregate",
                "_run_swarm_fan_in_aggregate",
            ),
        ),
    ]
...
def _build_latex_runners(executor: Any) -> list[DeterministicJobRunner]:
    return [
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "latex_runner_service",
                "run_latex_citation_sync",
                "_run_latex_citation_sync",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "latex_runner_service",
                "run_latex_reviewer_critic",
                "_run_latex_reviewer_critic",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "latex_runner_service",
                "run_latex_compile_project",
                "_run_latex_compile_project",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "latex_runner_service",
                "run_latex_publish_project",
                "_run_latex_publish_project",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "latex_runner_service",
                "run_latex_apply_unified_diff",
                "_run_latex_apply_unified_diff",
            ),
        ),
    ]
...
def _build_experiment_runners(executor: Any) -> list[DeterministicJobRunner]:
    return [
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "experiment_runner_service",
                "run_experiment_loop_seed",
                "_run_experiment_loop_seed",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "experiment_runner_service",
                "run_experiment_plan_generate",
                "_run_experiment_plan_generate",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "experiment_runner_service",
                "run_experiment_decide_next",
                "_run_experiment_decide_next",
            ),
        ),
        FunctionDeterministicJobRunner(
            "experiment_runner",
        ...
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "experiment_runner_service",
                "run_experiment_persist_results",
                "_run_experiment_persist_results",
            ),
        ),
    ]
...
def _build_coding_runners(executor: Any) -> list[DeterministicJobRunner]:
    return [
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "coding_runner_service",
                "run_code_patch_proposer",
                "_run_code_patch_proposer",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "coding_runner_service",
                "run_code_patch_apply_to_kb",
                "_run_code_patch_apply_to_kb",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "coding_runner_service",
                "run_coding_backlog_orchestrator",
                "_run_coding_backlog_orchestrator",
            ),
        ),
    ]
...
def _build_ingestion_demo_runners(executor: Any) -> list[DeterministicJobRunner]:
    return [
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "ingestion_demo_runner_service",
                "run_arxiv_inbox_extract_repos",
                "_run_arxiv_inbox_extract_repos",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "ingestion_demo_runner_service",
                "run_git_repo_ingest_wait",
                "_run_git_repo_ingest_wait",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "ingestion_demo_runner_service",
                "run_paper_algorithm_project",
                "_run_paper_algorithm_project",
            ),
        ),
        FunctionDeterministicJobRunner(
            ...
            _resolve_handler(
                executor,
                "ingestion_demo_runner_service",
                "run_generated_project_demo_check",
                "_run_generated_project_demo_check",
            ),
        ),
    ]
