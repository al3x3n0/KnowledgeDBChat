...
async def _validate_profile_ids(
    db: AsyncSession, user_id: UUID, profile_ids: list[str]
) -> None:
    if not profile_ids:
        return
    ...
    if missing:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown domain profile ids: {', '.join(missing[:5])}",
        )


async def _validate_sandbox_profile(
    db: AsyncSession, sandbox_profile_id: str | None
) -> None:
    requested = str(sandbox_profile_id or "").strip()
    if not requested:
        return
    profile = await get_scientific_sandbox_profile(
        db, requested, include_disabled=False
    )
...
async def _create_portfolio_job(
    *,
    ...
    job = AgentJob(
        ...
        schedule_cron=None,
        next_run_at=datetime.utcnow() + timedelta(minutes=1440)
...
async def _portfolio_response(
    portfolio: ResearchPortfolio, db: AsyncSession
) -> ResearchPortfolioResponse:
    automation_profile = normalize_portfolio_automation_profile(
    ...
    summary = (
        portfolio.latest_summary if isinstance(portfolio.latest_summary, dict) else {}
    )
    ...
    opportunities = list_normalized_research_opportunities(
        portfolio.opportunities, validation_status_by_id=validation_status_by_id
    )
...
async def _resolve_portfolio_parent_job(
        ...
        plan={
            ...
            "provenance": {
                "autonomous_origin": {
                    ...
                    "source_id": str(
                        generator_details.get("portfolio_id") or ""
                    ).strip()
                    or None,
...
async def _sync_portfolio_opportunities(
    portfolio: ResearchPortfolio, opportunities: list[dict]
) -> None:
        ...
        dict.fromkeys(
            [
                *(
                    [
                        str(v)
                        for v in (portfolio.latest_note_ids or [])
                        if str(v).strip()
                    ]
                ),
                *linked_ids["note_ids"],
...
async def list_research_portfolios(
    ...
    total_stmt = select(ResearchPortfolio.id).where(
        ResearchPortfolio.user_id == current_user.id
    )
    if str(status_filter or "").strip():
        total_stmt = total_stmt.where(
            ResearchPortfolio.status == str(status_filter).strip()
        )
...
@router.post(
    "", response_model=ResearchPortfolioResponse, status_code=status.HTTP_201_CREATED
)
async def create_research_portfolio(
    ...
    portfolio = ResearchPortfolio(
        ...
        automation_profile=normalize_portfolio_automation_profile(
            payload.automation_profile, default="balanced"
        ),
        automation_policy=resolve_portfolio_automation_policy(
            payload.automation_profile, payload.automation_policy
        ),
...
async def update_research_portfolio(
    ...
    await _validate_sandbox_profile(
        db, updates.get("sandbox_profile_id", portfolio.sandbox_profile_id)
    )
    ...
    if "automation_profile" in updates or "automation_policy" in updates:
        automation_profile = normalize_portfolio_automation_profile(
            updates.get(
                "automation_profile", getattr(portfolio, "automation_profile", None)
            ),
            default="balanced",
        )
...
async def act_on_research_portfolio(
    ...
    active_job = (
        await db.get(AgentJob, portfolio.active_job_id)
        if portfolio.active_job_id
        else None
    )
    ...
    if action == "start":
        ...
        await _create_portfolio_job(
            db=db, portfolio=portfolio, schedule_type="once", start_immediately=True
        )
    else:
...
@router.post(
    "/{portfolio_id}/opportunities/{opportunity_id}/action",
    response_model=ResearchPortfolioResponse,
)
async def act_on_research_portfolio_opportunity(
    ...
    async def _ensure_plan_ids(current_plan_ids: list[str]) -> list[str]:
        ...
        plan_id = await _create_plan_for_opportunity(
            ...
            or [str(v) for v in (portfolio.latest_note_ids or []) if str(v).strip()],
    ...
    if action == "accept":
        ...
        materialization = await materialize_research_opportunity_experiment(
            ...
            validation_policy=portfolio.automation_policy
            if isinstance(portfolio.automation_policy, dict)
            else {},
            sandbox_profile_id=portfolio.sandbox_profile_id,
            repo_source_ids=[
                str(v)
                for v in (opportunity.get("source_repo_ids") or [])
                if str(v).strip()
            ],
            benchmark_queries=[],
        ...
        if existing_child_job_ids:
            ...
            opportunity["follow_up_review_evidence_revision"] = opportunity.get(
                "evidence_revision"
            )
        ...
        child_job = await executor._create_domain_research_follow_up_job(
            ...
            if opportunity.get("source_repo_ids")
            else "kb_plus_arxiv",
