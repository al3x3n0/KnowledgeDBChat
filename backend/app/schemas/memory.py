...
class MemoryBase(BaseModel):
    ...
    memory_type: str = Field(
        ..., description="Type of memory: fact, preference, context, summary"
    )
    content: str = Field(..., description="Memory content")
    importance_score: float = Field(
        0.5, ge=0.0, le=1.0, description="Importance score (0.0 to 1.0)"
    )
...
class MemoryInteractionBase(BaseModel):
    ...
    interaction_type: str = Field(
        ...,
        description="Type of interaction: retrieved, updated, reinforced, contradicted",
    )
...
class UserPreferencesBase(BaseModel):
    ...
    max_memories_per_session: int = Field(
        10, ge=1, le=100, description="Max memories to load per session"
    )
    memory_importance_threshold: float = Field(
        0.3, ge=0.0, le=1.0, description="Minimum importance to store"
    )
    auto_summarize_sessions: bool = Field(
        True, description="Auto-create session summaries"
    )
    auto_memory_build_enabled: bool = Field(
        True, description="Enable automatic memory extraction while chatting"
    )
    auto_memory_build_mode: str = Field(
        "per_turn",
        description="Automatic extraction mode: off, manual, per_turn, periodic",
    )
    auto_memory_build_min_messages: int = Field(
        3,
        ge=1,
        le=100,
        description="Periodic mode: minimum new turns before extraction",
    )
    auto_memory_build_min_minutes: int = Field(
        10,
        ge=1,
        le=1440,
        description="Periodic mode: minimum minutes between extractions",
    )
    allow_cross_session_memory: bool = Field(
        True, description="Share memories across sessions"
    )
    allow_personal_data_storage: bool = Field(
        True, description="Store personal information"
    )

    # LLM preferences (per-user overrides)
    llm_provider: Optional[str] = Field(
        None, description="LLM provider override: ollama, deepseek, openai, or custom"
    )
    llm_model: Optional[str] = Field(
        None, description="Model name override (default for chat)"
    )
    llm_api_url: Optional[str] = Field(None, description="Custom API URL override")
    llm_api_key: Optional[str] = Field(
        None, description="User's own API key for external providers"
    )
    llm_temperature: Optional[float] = Field(
        None, ge=0.0, le=2.0, description="Temperature override"
    )
    llm_max_tokens: Optional[int] = Field(
        None, ge=1, le=32000, description="Max response tokens override"
    )
    ...
    paper_algo_default_run_demo_check: bool = Field(
        False,
        description="Default: run demo check after generating paper algorithm projects (when available)",
    )
...
class UserPreferencesUpdate(BaseModel):
    ...
    auto_memory_build_mode: Optional[str] = Field(
        None, description="off, manual, per_turn, periodic"
    )
    ...
    # Per-task model overrides
    llm_task_models: Optional[Dict[str, str]] = Field(
        None, description="Task-specific model overrides"
    )
...
class MemorySummaryRequest(BaseModel):
    ...
    time_range_days: Optional[int] = Field(
        7, ge=1, le=30, description="Time range in days"
    )
    include_types: Optional[List[str]] = Field(
        None, description="Memory types to include"
    )
