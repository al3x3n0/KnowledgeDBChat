...
class ResearchMonitorProfileService:
    ...
    @staticmethod
    def _score_trend(
        *, completed_count: int, failed_count: int, cancelled_count: int
    ) -> str:
    ...
    def _normalize_allowed_recommendations(self, raw: object) -> list[str]:
        return normalize_monitor_allowed_recommendations(
            raw, default_allowed=self.SAFE_AUTONOMY_RECOMMENDATIONS
        )
    ...
    def _normalize_policy_config(self, raw: object) -> dict[str, Any]:
        ...
        return {
            ...
            "allowed_recommendations": self._normalize_allowed_recommendations(
    ...
    def _normalize_budget_config(
        self, raw: object, *, defaults: Optional[dict[str, int]] = None
    ) -> dict[str, int]:
    ...
    def _normalize_customer_budget_config(self, raw: object) -> dict[str, int]:
        return self._normalize_budget_config(
            raw, defaults=self.DEFAULT_CUSTOMER_AUTONOMY_BUDGET
        )
    ...
    def _normalize_budget_history_entry(self, raw: object) -> Optional[dict[str, Any]]:
        ...
        return {
            ...
            "previous_autonomy_budget": self._normalize_budget_config(
                raw.get("previous_autonomy_budget")
            ),
            "next_autonomy_budget": self._normalize_budget_config(
                raw.get("next_autonomy_budget")
            ),
            "guidance_context": raw.get("guidance_context")
            if isinstance(raw.get("guidance_context"), dict)
            else {},
        }

    def _normalize_customer_rebalance_history_entry(
        self, raw: object
            ...
            changes.append(
                {
                    ...
                    "monitor_name": str(change.get("monitor_name") or "").strip()
                    or "Unknown monitor",
                    "customer": (str(change.get("customer") or "").strip() or None),
                    "current_budget": self._normalize_budget_config(
                        change.get("current_budget")
                    ),
                    "proposed_budget": self._normalize_budget_config(
                        change.get("proposed_budget")
                    ),
                    "delta_budget": {
                        "auto_launch_limit_24h": int(
                            (change.get("delta_budget") or {}).get(
                                "auto_launch_limit_24h", 0
                            )
                            or 0
                        ),
                        "approval_queue_limit_24h": int(
                            (change.get("delta_budget") or {}).get(
                                "approval_queue_limit_24h", 0
                            )
                            or 0
                        ...
                        "alert_limit_24h": int(
                            (change.get("delta_budget") or {}).get("alert_limit_24h", 0)
                            or 0
                        ),
                        "queue_backlog_cap": int(
                            (change.get("delta_budget") or {}).get(
                                "queue_backlog_cap", 0
                            )
                            or 0
        ...
        return {
            ...
            "actor_user_id": (str(raw.get("actor_user_id") or "").strip() or None),
            "change_source": (
                str(raw.get("change_source") or "").strip()
                or "customer_rebalance_guidance"
            ),
            "change_reason": (str(raw.get("change_reason") or "").strip() or None),
            "changes": changes,
            "before_capacity": self._normalize_budget_config(
                raw.get("before_capacity"),
                defaults={
                    "auto_launch_limit_24h": 0,
                    "approval_queue_limit_24h": 0,
                    "alert_limit_24h": 0,
            ...
            "after_capacity": self._normalize_budget_config(
                raw.get("after_capacity"),
                defaults={
                    "auto_launch_limit_24h": 0,
                    "approval_queue_limit_24h": 0,
                    "alert_limit_24h": 0,
            ...
            "evaluation_target_count": max(
                3,
                int(
                    raw.get("evaluation_target_count")
                    or self.CUSTOMER_REBALANCE_EVALUATION_TARGET_COUNT
                ),
            ),
            "evaluation_state": (
    ...
    def _customer_rebalance_history_for_profile(
        self, profile: Optional[ResearchMonitorProfile]
    ) -> list[dict[str, Any]]:
        if not profile:
            return []
        ...
        *,
        ...
        for change in changes:
            ...
            normalized_changes.append(
                {
                    "monitor_job_id": str(change.get("monitor_job_id") or "").strip()
                    or None,
                    "monitor_name": str(change.get("monitor_name") or "").strip()
                    or "Unknown monitor",
                    "customer": (str(change.get("customer") or "").strip() or None),
        ...
        entry = {
            ...
            "change_source": (
                str(change_source or "").strip() or "customer_rebalance_guidance"
            ),
    ...
    def append_budget_history_entry(
        ...
        *,
        ...
        entry = {
            ...
            if isinstance(guidance_context, dict)
            else {},
        }
        history.insert(0, entry)
        normalized = sorted(history, key=lambda row: row["at"], reverse=True)[:20]
    ...
    def _budget_remaining(
        self, budget_config: dict[str, int], budget_usage: dict[str, int]
    ) -> dict[str, int]:
        return {
            "auto_launch_count_24h": max(
                0,
                int(budget_config.get("auto_launch_limit_24h", 0) or 0)
                - int(budget_usage.get("auto_launch_count_24h", 0) or 0),
            ),
            "approval_queue_count_24h": max(
                0,
                int(budget_config.get("approval_queue_limit_24h", 0) or 0)
                - int(budget_usage.get("approval_queue_count_24h", 0) or 0),
            ),
            "alert_count_24h": max(
                0,
                int(budget_config.get("alert_limit_24h", 0) or 0)
                - int(budget_usage.get("alert_count_24h", 0) or 0),
            ),
            "queue_backlog_count": max(
                0,
                int(budget_config.get("queue_backlog_cap", 0) or 0)
                - int(budget_usage.get("queue_backlog_count", 0) or 0),
            ),
        }
    ...
    def _build_customer_rebalance_guidance(
        ...
        *,
        ...
        customer_throttle = (
            str(customer_row.get("customer_budget_throttle_state") or "normal")
            .strip()
            .lower()
        )
        portfolio_status = (
            str(customer_row.get("portfolio_status") or "normal").strip().lower()
        )
        actionable = customer_throttle != "normal" or portfolio_status in {
            "monitor_throttled",
            "backlog_heavy",
            "alert_heavy",
            "nearing_saturation",
            "customer_budget_throttled",
        }
        if not actionable:
            return "none", [], None, []
        ...
        ranked_pressure = sorted(
            ...
            key=lambda row: (
                -(
                    self._budget_ratio(
                        row.get("budget_usage") or {},
                        row.get("autonomy_budget") or {},
                        "queue_backlog_count",
                        "queue_backlog_cap",
                    )
                    + self._budget_ratio(
                        row.get("budget_usage") or {},
                        row.get("autonomy_budget") or {},
                        "approval_queue_count_24h",
                        "approval_queue_limit_24h",
                    )
                    + self._budget_ratio(
                        row.get("budget_usage") or {},
                        row.get("autonomy_budget") or {},
                        "auto_launch_count_24h",
                        "auto_launch_limit_24h",
                    )
                ),
                0
                if str(row.get("health_bucket") or "") == "weak"
                else 1
                if str(row.get("health_bucket") or "") == "mixed"
                else 2,
        ...
        relief_monitor = next(
            (
                row
                for row in ranked_relief
                if row.get("monitor_job_id") != pressure_monitor.get("monitor_job_id")
            ),
            None,
        )
        ...
        field_specs = [
            ...
            (
                "approval_queue_limit_24h",
                "approval_queue_count_24h",
                "approval queue headroom",
            ),
            ("queue_backlog_cap", "queue_backlog_count", "backlog headroom"),
            ("alert_limit_24h", "alert_count_24h", "alert headroom"),
        ]
        pressure_budget = self._normalize_budget_config(
            pressure_monitor.get("autonomy_budget")
        )
        relief_budget = self._normalize_budget_config(
            relief_monitor.get("autonomy_budget")
        )
        pressure_usage = (
            pressure_monitor.get("budget_usage")
            if isinstance(pressure_monitor.get("budget_usage"), dict)
            else {}
        )
        relief_usage = (
            relief_monitor.get("budget_usage")
            if isinstance(relief_monitor.get("budget_usage"), dict)
            else {}
        )
        ...
        for budget_key, usage_key, label in field_specs:
            pressure_ratio = self._budget_ratio(
                pressure_usage, pressure_budget, usage_key, budget_key
            )
            relief_ratio = self._budget_ratio(
                relief_usage, relief_budget, usage_key, budget_key
            )
            if pressure_ratio < 0.8 or relief_ratio > 0.5:
                continue
            if int(pressure_budget.get(budget_key, 0) or 0) <= 1:
                continue
            next_pressure_budget[budget_key] = max(
                1, int(next_pressure_budget.get(budget_key, 0) or 0) - 1
            )
            next_relief_budget[budget_key] = (
                int(next_relief_budget.get(budget_key, 0) or 0) + 1
            )
            pressure_reasons.append(
                f"Reduce {label} on {pressure_monitor.get('monitor_name')}."
            )
            relief_reasons.append(
                f"Reassign {label} to {relief_monitor.get('monitor_name')}."
            )
            ...
            changes.append(
                {
                    ...
                    "delta_budget": {
                        key: int(next_pressure_budget.get(key, 0) or 0)
                        - int(pressure_budget.get(key, 0) or 0)
                        for key in self.DEFAULT_AUTONOMY_BUDGET.keys()
                    },
                    "reasons": pressure_reasons
                    or ["This monitor is driving disproportionate portfolio pressure."],
                }
            )
            ...
            changes.append(
                {
                    ...
                    "delta_budget": {
                        key: int(next_relief_budget.get(key, 0) or 0)
                        - int(relief_budget.get(key, 0) or 0)
                        for key in self.DEFAULT_AUTONOMY_BUDGET.keys()
                    },
                    "reasons": relief_reasons
                    or ["This monitor is strong and currently under-utilized."],
                }
            )
    ...
    def _customer_capacity_from_changes(
        ...
        *,
        ...
        for row in base_rows:
            ...
            budget = changes_by_monitor.get(
                monitor_id
            ) or self._normalize_budget_config(row.get("autonomy_budget"))
    ...
    def _budget_usage_for_monitor(
        ...
        *,
        ...
        for item in items:
            ...
            operator_decision = (
                str(item.follow_up_operator_decision or "").strip().lower()
            )
            ...
            if launch_status in {
                ...
                "blocked",
                "failed",
            } or decision in {"manual", "queued_for_approval", "launch_failed"}:
                if operator_decision not in {
                    "rejected",
                    "approved_launch",
                } and launch_status not in {"rejected", "launched"}:
    ...
    async def build_monitor_budget_snapshot(
        ...
        *,
        ...
        effective_policy = (
            automation["effective_policy"]
            if isinstance(automation.get("effective_policy"), dict)
            else {}
        )
        ...
        for notification in notifications:
            ...
            if str(data.get("job_id") or "").strip() == str(monitor_job.id):
                alert_count += 1
        usage = self._budget_usage_for_monitor(
            items=inbox_rows, alert_count_24h=alert_count, now=now
        )
        remaining = self._budget_remaining(budget_config, usage)
        throttle_state, throttle_reasons = self._derive_budget_throttle(
            policy_mode=str(
                effective_policy.get("follow_up_review_mode") or "manual_only"
            ),
        ...
        *,
        ...
        if not customer_name:
            ...
            return {
                ...
                "customer_budget_remaining": self._budget_remaining(
                    budget_config, usage
                ),
        ...
        usage = self._budget_usage_for_monitor(
            items=inbox_rows, alert_count_24h=alert_count, now=current_time
        )
        remaining = self._budget_remaining(budget_config, usage)
        throttle_state, throttle_reasons = self._derive_budget_throttle(
    ...
    def _policy_history_for_job(self, job: Optional[AgentJob]) -> list[dict[str, Any]]:
        ...
        for raw in raw_entries:
            ...
            entries.append(
                {
                    ...
                    "at": parsed_at,
                    "actor_user_id": (
                        str(raw.get("actor_user_id") or "").strip() or None
                    ),
                    "change_source": (
                        str(raw.get("change_source") or "").strip() or None
                    ),
                    "change_reason": (
                        str(raw.get("change_reason") or "").strip() or None
                    ),
                    "previous_automation_profile": normalize_portfolio_automation_profile(
                        raw.get("previous_automation_profile"), default="balanced"
                    ),
                    "next_automation_profile": normalize_portfolio_automation_profile(
                        raw.get("next_automation_profile"), default="balanced"
                    ),
                    "previous_automation_policy": raw.get("previous_automation_policy")
                    if isinstance(raw.get("previous_automation_policy"), dict)
                    else {},
                    "next_automation_policy": raw.get("next_automation_policy")
                    if isinstance(raw.get("next_automation_policy"), dict)
                    else {},
                    "previous_effective_policy": raw.get("previous_effective_policy")
                    if isinstance(raw.get("previous_effective_policy"), dict)
                    else {},
                    "next_effective_policy": raw.get("next_effective_policy")
                    if isinstance(raw.get("next_effective_policy"), dict)
                    else {},
                    ...
                    "analytics_context": raw.get("analytics_context")
                    if isinstance(raw.get("analytics_context"), dict)
                    else {},
                    "evaluation_target_count": max(
                        2,
                        int(
                            raw.get("evaluation_target_count")
                            or self.POLICY_EVALUATION_TARGET_COUNT
                        ),
                    ),
                    "evaluation_state": (
                        str(raw.get("evaluation_state") or "").strip().lower()
                        or "active"
                    ),
                }
            )
            entries[-1].update(
                build_monitor_policy_history_compat_entry(
                    previous_snapshot={
                        "follow_up_autonomy": self._normalize_policy_config(
                            raw.get("previous_follow_up_autonomy")
                        ),
                    },
                    next_snapshot={
                        "follow_up_autonomy": self._normalize_policy_config(
                            raw.get("next_follow_up_autonomy")
                        ),
                    },
                )
            )
    ...
    @staticmethod
    def _customer_portfolio_status_for_bucket(
        bucket: dict[str, Any]
    ) -> tuple[str, list[str]]:
        reasons: list[str] = []
        ...
        if throttled_monitor_count > 0:
            reasons.append(
                f"{throttled_monitor_count} monitor(s) are currently throttled."
            )
        if backlog_capacity > 0 and backlog_used >= _saturation_floor(backlog_capacity):
            reasons.append(
                f"Backlog is using {backlog_used}/{backlog_capacity} available headroom."
            )
        if alert_capacity > 0 and alert_used >= _saturation_floor(alert_capacity):
            reasons.append(
                f"Alerts used {alert_used}/{alert_capacity} of configured headroom."
            )
        if (auto_capacity > 0 and auto_used >= _saturation_floor(auto_capacity)) or (
            approval_capacity > 0
            and approval_used >= _saturation_floor(approval_capacity)
        ):
    ...
    def _policy_evaluation_counts_for_items(
        self, items: list[ResearchInboxItem]
    ) -> dict[str, int]:
        ...
        *,
        ...
        return {
            ...
            "follow_up_completed_count": int(
                policy_counts["follow_up_completed_count"]
            ),
            "follow_up_failed_count": int(policy_counts["follow_up_failed_count"]),
            "follow_up_cancelled_count": int(
                policy_counts["follow_up_cancelled_count"]
            ),
            "auto_launch_used_24h": sum(
                int(
                    (row.get("budget_usage") or {}).get("auto_launch_count_24h", 0) or 0
                )
                for row in monitor_rows
            ),
            "approval_queue_used_24h": sum(
                int(
                    (row.get("budget_usage") or {}).get("approval_queue_count_24h", 0)
                    or 0
                )
                for row in monitor_rows
            ),
            "alert_used_24h": sum(
                int((row.get("budget_usage") or {}).get("alert_count_24h", 0) or 0)
                for row in monitor_rows
            ),
            "backlog_used": sum(
                int((row.get("budget_usage") or {}).get("queue_backlog_count", 0) or 0)
                for row in monitor_rows
            ),
            "throttled_monitor_count": sum(
                1
                for row in monitor_rows
                if str(row.get("budget_throttle_state") or "normal").strip().lower()
                != "normal"
            ),
        }

    def _customer_rebalance_delta_counts(
        self, before_counts: dict[str, int], after_counts: dict[str, int]
    ) -> dict[str, int]:
        return {
            key: int(after_counts.get(key, 0) or 0)
            - int(before_counts.get(key, 0) or 0)
            ...
            samples.append(
                {
                    ...
                    "period": period,
                    "launch_status": (
                        str(item.follow_up_launch_status or "").strip().lower() or None
                    ),
                    "outcome_status": (
                        str(item.follow_up_outcome_status or "").strip().lower() or None
                    ),
                    "recommendation_key": (
                        str(item.follow_up_recommendation_key or "").strip() or None
                    ),
                    "summary": summary,
    ...
    @staticmethod
    def _policy_evaluation_delta_counts(
    ...
    def _evaluate_policy_history_entry(
        ...
        *,
        ...
        target_count = max(
            2,
            int(
                history_entry.get("evaluation_target_count")
                or self.POLICY_EVALUATION_TARGET_COUNT
            ),
        )
        ...
        before_candidates = [
            item for item in sorted_items if self._item_sort_time(item) < effective_at
        ]
        after_candidates = [
            item for item in sorted_items if self._item_sort_time(item) >= effective_at
        ]
        ...
        before_completion_rate = self._safe_rate(
            before_counts["follow_up_completed_count"], before_counts["accepted_count"]
        )
        after_completion_rate = self._safe_rate(
            after_counts["follow_up_completed_count"], after_counts["accepted_count"]
        )
        before_negative = (
        ...
        before_block_rate = self._safe_rate(
            before_counts["blocked_count"], before_counts["accepted_count"]
        )
        after_block_rate = self._safe_rate(
            after_counts["blocked_count"], after_counts["accepted_count"]
        )
        reasons: list[str] = []
        ...
        if sample_count < min(target_count, 3):
            ...
            reasons.append(
                f"Only {sample_count} accepted signal(s) observed after this policy change"
            )
        else:
            improving_signals = 0
            degrading_signals = 0
            if after_completion_rate > before_completion_rate:
                improving_signals += 1
                reasons.append(
                    f"Completion rate improved from {before_completion_rate:.1f}% to {after_completion_rate:.1f}%"
                )
            elif after_completion_rate < before_completion_rate:
                degrading_signals += 1
                reasons.append(
                    f"Completion rate fell from {before_completion_rate:.1f}% to {after_completion_rate:.1f}%"
                )
            ...
            if not reasons:
                reasons.append(
                    "Launch mix and downstream outcomes are roughly flat after this policy change"
                )
        ...
        return {
            ...
            "sample_items": [
                *self._policy_evaluation_samples_for_items(
                    list(reversed(before_items)), period="before"
                ),
    ...
    def _evaluate_customer_rebalance_history_entry(
        ...
        *,
        ...
        before_monitor_rows = [
            row
            for row in monitor_rows
            if str(row.get("monitor_job_id") or "").strip() in changed_monitor_ids
        ]
        if not before_monitor_rows:
            before_monitor_rows = monitor_rows
        after_monitor_rows = monitor_rows

        before_counts = self._customer_rebalance_evaluation_counts(
            items=before_items, monitor_rows=before_monitor_rows
        )
        after_counts = self._customer_rebalance_evaluation_counts(
            items=after_items, monitor_rows=after_monitor_rows
        )
        delta_counts = self._customer_rebalance_delta_counts(
        ...
        if sample_count < min(target_count, 2):
            status = "insufficient_data"
            reasons.append(
                f"Only {sample_count} accepted signal(s) observed after this rebalance"
            )
        else:
            improving_signals = 0
            degrading_signals = 0
            if int(after_counts["throttled_monitor_count"]) < int(
                before_counts["throttled_monitor_count"]
            ):
                improving_signals += 1
                reasons.append("Fewer monitors are throttled after the rebalance")
            elif int(after_counts["throttled_monitor_count"]) > int(
                before_counts["throttled_monitor_count"]
            ):
                degrading_signals += 1
            ...
            elif int(after_counts["blocked_count"]) > int(
                before_counts["blocked_count"]
            ):
                degrading_signals += 1
                reasons.append("More accepted signals are being blocked")
            if int(after_counts["follow_up_completed_count"]) > int(
                before_counts["follow_up_completed_count"]
            ):
                improving_signals += 1
                reasons.append("More launched follow-ups are completing")
            elif int(after_counts["follow_up_failed_count"]) + int(
                after_counts["follow_up_cancelled_count"]
            ) > int(before_counts["follow_up_failed_count"]) + int(
                before_counts["follow_up_cancelled_count"]
            ):
                degrading_signals += 1
            ...
            if not reasons:
                reasons.append(
                    "Portfolio pressure and downstream outcomes are roughly flat after this rebalance"
                )
        ...
        return {
            ...
            "delta_counts": delta_counts,
            "sample_items": [
                *self._customer_rebalance_sample_items(
                    list(reversed(before_items)), period="before", jobs_by_id=jobs_by_id
                ),
                *self._customer_rebalance_sample_items(
                    after_items, period="after", jobs_by_id=jobs_by_id
                ),
            ],
        }
        ...
        *,
        ...
        if mode != "manual_only":
            for action in actions:
                ...
                eligibility = (
                    str(action.autonomy_eligibility or "").strip().lower()
                    or "manual_only"
                )
        ...
        recommendation_key = (
            str(selected_action.recommendation_key or "").strip() or None
        )
        eligibility = (
            str(selected_action.autonomy_eligibility or "").strip().lower()
            or "manual_only"
        )
    ...
    def build_policy_simulation_snapshot(
        ...
        *,
        ...
        proposed_follow_up_autonomy = (
            ...
            if isinstance(proposed_raw.get("follow_up_autonomy"), dict)
            else (
                {
                    ...
                    "allowed_recommendations": proposed_raw.get(
                        "allowed_recommendations"
                    ),
                }
                if proposed_raw.get("mode") is not None
                or proposed_raw.get("allowed_recommendations") is not None
                else current_policy
            )
        )
        proposed_uses_raw_mode = not isinstance(
            proposed_raw.get("automation_policy"), dict
        ) and (
            proposed_raw.get("mode") is not None
            or isinstance(proposed_raw.get("follow_up_autonomy"), dict)
        )
        ...
        proposed_automation = self.resolve_monitor_automation_config(
            {
                **current_config,
                ...
                "automation_profile": proposed_raw.get(
                    "automation_profile", current_automation["automation_profile"]
                ),
        ...
        for item in items[:history_limit]:
            ...
            if (
                baseline.get("recommendation_key")
                and baseline["classification"] != "insufficient_context"
            ):
                baseline_recommendations[str(baseline["recommendation_key"])] += 1
            if (
                simulated.get("recommendation_key")
                and simulated["classification"] != "insufficient_context"
            ):
            ...
            if len(sample_items) < 6 and (
                ...
                or baseline.get("recommendation_key")
                != simulated.get("recommendation_key")
            ):
                sample_items.append(
                    {
                        "item_id": item.id,
                        "title": str(item.title or "").strip() or "Untitled inbox item",
                        "recommendation_key": simulated.get("recommendation_key")
                        or baseline.get("recommendation_key"),
        ...
        recommendation_keys = sorted(
            set(baseline_recommendations.keys()) | set(simulated_recommendations.keys())
        )
        recommendation_deltas = [
            {
                ...
                "delta_count": int(simulated_recommendations.get(key) or 0)
                - int(baseline_recommendations.get(key) or 0),
            }
        ...
        return {
            ...
            "insufficient_context_count": int(
                simulated_counts["insufficient_context_count"]
            ),
        }
    ...
    async def build_policy_simulation(
        ...
        *,
        ...
        learning_profile = await load_follow_up_learning_profile(
            db=db,
            user_id=user_id,
    ...
    def _health_summary_for_bucket(
        ...
        score = (
            (acceptance_rate * 0.45) + (completion_rate * 0.35) + (autonomy_rate * 0.20)
        )
        reasons: list[str] = []
    ...
    def _policy_guidance_for_bucket(
        self, bucket: dict[str, Any]
    ) -> tuple[str, list[str], list[str], str]:
        effective_policy = (
            bucket.get("effective_policy")
            if isinstance(bucket.get("effective_policy"), dict)
            else {}
        )
        automation_policy = (
            bucket.get("automation_policy")
            if isinstance(bucket.get("automation_policy"), dict)
            else {}
        )
        current_mode = (
            str(
        ...
        latest_evaluation_status = (
            str(bucket.get("latest_policy_evaluation_status") or "").strip().lower()
        )
        latest_evaluation_sample_count = int(
            bucket.get("latest_policy_evaluation_sample_count", 0) or 0
        )
        ...
        if (
            latest_evaluation_status == "degrading"
            and latest_evaluation_sample_count >= 3
        ):
            recommended_mode = (
                "queue_for_approval"
            ...
            policy_reasons.append(
                "Latest policy change is degrading follow-up outcomes"
            )
        elif (
            health_bucket == "strong"
            and acceptance_rate >= 60.0
            and completed >= max(2, failed + cancelled)
        ):
            recommended_mode = "auto_launch_safe"
            policy_reasons.append(
                "Discovery quality and follow-up outcomes support safe auto-launch"
            )
        elif (
            health_bucket == "weak"
            or failed > completed
            or blocked >= max(2, accepted // 2 or 1)
        ):
            ...
            if blocked >= max(2, accepted // 2 or 1):
                ...
                policy_reasons.append(
                    "Recent follow-up outcomes are too weak for autonomy"
                )
        else:
            recommended_mode = "queue_for_approval"
            policy_reasons.append(
                "Signals are useful, but follow-ups still need operator confirmation"
            )
        ...
        for key in self.SAFE_AUTONOMY_RECOMMENDATIONS:
            row = next(
                (
                    candidate
                    for candidate in recommendation_rows
                    if candidate.get("recommendation_key") == key
                ),
                None,
            )
            if row is None:
        ...
        if recommended_mode == "manual_only":
            recommended_allowed = (
                recommended_allowed
                or current_allowed
                or list(self.SAFE_AUTONOMY_RECOMMENDATIONS)
            )
        ...
        if recommended_mode == current_mode and recommended_allowed == current_allowed:
            policy_reasons.append(
                "Current policy already matches observed monitor performance"
            )
    ...
    def _policy_guardrail_for_bucket(
        self, bucket: dict[str, Any]
    ) -> tuple[
        Optional[str], Optional[str], list[str], Optional[str], Optional[dict[str, Any]]
    ]:
        latest_status = (
            str(bucket.get("latest_policy_evaluation_status") or "").strip().lower()
        )
        ...
        current_policy = self._normalize_policy_config(
            {
                "mode": effective_policy.get("follow_up_review_mode")
                or automation_policy.get("follow_up_review_mode"),
                "allowed_recommendations": (
                    effective_policy.get("allowed_recommendations")
                    if isinstance(effective_policy.get("allowed_recommendations"), list)
        ...
        previous_policy = self._normalize_policy_config(
            latest_entry.get("previous_follow_up_autonomy")
        )
        reasons = list(bucket.get("latest_policy_evaluation_reasons") or [])[:2]
        if reasons:
            reasons = [
                *reasons,
                "Apply a more conservative policy until outcomes recover",
            ][:3]
        else:
            reasons = [
                "Latest policy change is degrading follow-up outcomes",
        ...
        current_mode = (
            str(current_policy.get("mode") or "manual_only").strip().lower()
            or "manual_only"
        )
        if current_mode == "auto_launch_safe":
            next_policy = {
                ...
                "allowed_recommendations": list(
                    current_policy.get("allowed_recommendations")
                    or list(self.SAFE_AUTONOMY_RECOMMENDATIONS)
                ),
            }
        ...
        *,
        ...
        for item in items:
            ...
            monitor_key = (
                str(item.job_id)
                if item.job_id
                else f"unattributed:{customer or 'global'}"
            )
            monitor_bucket = monitor_buckets.setdefault(
                monitor_key,
                {
                    "monitor_job_id": item.job_id,
                    "monitor_name": str(job.name).strip()
                    if job and getattr(job, "name", None)
                    else "Unattributed inbox items",
                    "monitor_job_type": str(job.job_type).strip()
                    ...
                    "current_allowed_recommendations": list(
                        self.SAFE_AUTONOMY_RECOMMENDATIONS
                    ),
            ...
            if job and isinstance(getattr(job, "config", None), dict):
                ...
                automation_policy = (
                    ...
                    if isinstance(automation.get("automation_policy"), dict)
                    else {}
                )
                ...
                monitor_bucket["autonomy_budget"] = self._normalize_budget_config(
                    job.config.get("autonomy_budget")
                )
            if job:
                ...
                if budget_history:
                    monitor_bucket["budget_history_count"] = (
                        ...
                        and isinstance(job.results.get(self.BUDGET_HISTORY_KEY), list)
                        else len(budget_history)
                    )
                    monitor_bucket["latest_budget_changed_at"] = budget_history[0]["at"]
                    monitor_bucket["latest_budget_change_source"] = budget_history[
                        0
                    ].get("change_source")
                    monitor_bucket["latest_budget_actor_user_id"] = budget_history[
                        0
                    ].get("actor_user_id")
                    monitor_bucket["latest_budget_change_reason"] = budget_history[
                        0
                    ].get("change_reason")
                ...
                if policy_history:
                    monitor_bucket["policy_history_count"] = (
                        ...
                        if isinstance(getattr(job, "results", None), dict)
                        and isinstance(job.results.get(self.POLICY_HISTORY_KEY), list)
                        else len(policy_history)
                    )
                    monitor_bucket["latest_policy_changed_at"] = policy_history[0]["at"]
                    monitor_bucket["latest_policy_change_source"] = policy_history[
                        0
                    ].get("change_source")
                    monitor_bucket["latest_policy_actor_user_id"] = policy_history[
                        0
                    ].get("actor_user_id")
            ...
            customer_budget_decision = (
                str(item.follow_up_customer_budget_decision or "").strip().lower()
            )
            ...
            if launch_status in {"blocked", "rejected"} and decision in {
                "manual",
                "queued_for_approval",
            }:
        ...
        for bucket in monitor_buckets.values():
            bucket["acceptance_rate"] = self._safe_rate(
                bucket["accepted_count"], bucket["discovered_count"]
            )
            ...
            if bucket.get("monitor_job_id") and bucket.get("recent_policy_history"):
                ...
                bucket["latest_policy_evaluation_status"] = latest_evaluation.get(
                    "evaluation_status"
                )
                bucket["latest_policy_evaluation_sample_count"] = int(
                    latest_evaluation.get("evaluation_sample_count", 0) or 0
                )
                bucket["latest_policy_evaluation_target_count"] = int(
                    latest_evaluation.get("evaluation_target_count", 0) or 0
                )
                bucket["latest_policy_evaluation_reasons"] = list(
                    latest_evaluation.get("evaluation_reasons") or []
                )
            ...
            budget_usage = self._budget_usage_for_monitor(
                items=accepted_items,
                alert_count_24h=(notification_counts_by_job or {}).get(
                    str(bucket.get("monitor_job_id") or ""), 0
                ),
            )
            ...
            (
                budget_throttle_state,
                budget_throttle_reasons,
            ) = self._derive_budget_throttle(
                policy_mode=str(
                    (
                        (bucket.get("effective_policy") or {})
                        if isinstance(bucket.get("effective_policy"), dict)
                        else {}
                    ).get("follow_up_review_mode")
                    or "manual_only"
                ),
                budget_config=budget_config,
            ...
            bucket["budget_clamp_state"] = (
                None if budget_throttle_state == "normal" else budget_throttle_state
            )
            ...
            (
                ...
                health_bucket,
                health_reasons,
            ) = self._health_summary_for_bucket(bucket)
            ...
            bucket["policy_mode_counts"] = {
                str(k): int(v)
                for k, v in dict(bucket["policy_mode_counts"]).items()
                if int(v) > 0
            }
            bucket["follow_up_review_counts"] = {
                "auto_launch_safe": int(bucket.get("auto_launched_count", 0) or 0),
                "queue_for_approval": int(
                    bucket.get("queued_for_approval_count", 0) or 0
                ),
            ...
            bucket["scheduler_summary"] = {
                ...
                "queued_approvals_count": int(
                    bucket.get("queued_for_approval_count", 0) or 0
                ),
                "manual_recommendations_count": int(
                    bucket.get("manual_only_count", 0) or 0
                ),
                "blocked_by_policy_count": int(bucket.get("blocked_count", 0) or 0),
                "blocked_by_budget_count": 0
                if budget_throttle_state == "normal"
                else 1,
                "suppressed_relaunches_count": int(
                    bucket.get("suppressed_relaunches_count", 0) or 0
                ),
            }
            ...
            for recommendation in bucket["recommendation_counts"].values():
                success_rate = self._safe_rate(
                    ...
                    recommendation["completed_count"]
                    + recommendation["failed_count"]
                    + recommendation["cancelled_count"],
                )
        ...
        for row in monitor_rows:
            ...
            budget_usage = (
                row.get("budget_usage")
                if isinstance(row.get("budget_usage"), dict)
                else {}
            )
            budget_config = (
                row.get("autonomy_budget")
                if isinstance(row.get("autonomy_budget"), dict)
                else {}
            )
            ...
            approval_capacity = int(
                budget_config.get("approval_queue_limit_24h", 0) or 0
            )
            ...
            throttle_state = (
                str(row.get("budget_throttle_state") or "normal").strip().lower()
                or "normal"
            )
            ...
            bucket["follow_up_completed_count"] += int(
                row.get("follow_up_completed_count", 0) or 0
            )
            bucket["follow_up_failed_count"] += int(
                row.get("follow_up_failed_count", 0) or 0
            )
            bucket["follow_up_cancelled_count"] += int(
                row.get("follow_up_cancelled_count", 0) or 0
            )
            ...
            if throttle_state != "normal":
                ...
                bucket["_throttled_rows"].append(
                    {**contributor_base, "value": severity}
                )
        ...
        for bucket in customer_buckets.values():
            ...
            alert_capacity = int(bucket.get("alert_capacity_24h", 0) or 0)
            alert_used = int(bucket.get("alert_used_24h", 0) or 0)
            backlog_pressured = backlog_capacity > 0 and backlog_used >= max(
                1, math.ceil(backlog_capacity * 0.8)
            )
            alert_pressured = alert_capacity > 0 and alert_used >= max(
                1, math.ceil(alert_capacity * 0.8)
            )
            bucket["top_launch_monitors"] = sorted(
                [
                    row
                    for row in bucket.pop("_launch_rows", [])
                    if int(row.get("value", 0) or 0) > 0
                ],
                key=lambda row: (
            ...
            bucket["top_backlog_monitors"] = sorted(
                [
                    row
                    for row in bucket.pop("_backlog_rows", [])
                    if int(row.get("value", 0) or 0) > 0
                ]
            ...
            bucket["top_alert_monitors"] = sorted(
                [
                    row
                    for row in bucket.pop("_alert_rows", [])
                    if int(row.get("value", 0) or 0) > 0
                ]
                if alert_pressured
                else [],
            ...
            (
                ...
                monitor_rows=[
                    ...
                    == str(bucket.get("customer") or "").strip()
                ],
            )
            ...
            recommendation_rows.append(
                {
                    ...
                    "approval_launch_count": int(
                        recommendation["approval_launch_count"]
                    ),
        ...
        *,
        ...
        if job_ids:
            ...
            for notification in notif_res.scalars().all():
                ...
                if (
                    ...
                    or created_at.timestamp() < window_start
                ):
                    continue
                data = notification.data if isinstance(notification.data, dict) else {}
                job_key = str(data.get("job_id") or "").strip()
                if job_key:
                    notification_counts_by_job[job_key] = (
                        notification_counts_by_job.get(job_key, 0) + 1
                    )
        ...
        if customer_names:
            ...
            for row in snapshot.get("customers", []):
                ...
                usage = {
                    "auto_launch_count_24h": int(
                        row.get("auto_launch_used_24h", 0) or 0
                    ),
                    "approval_queue_count_24h": int(
                        row.get("approval_queue_used_24h", 0) or 0
                    ),
                ...
                row["customer_budget_remaining"] = self._budget_remaining(
                    budget_config, usage
                )
                ...
                if throttle_state != "normal":
                    existing_reasons = [
                        str(reason).strip()
                        for reason in (row.get("portfolio_reasons") or [])
                        if str(reason).strip()
                    ]
                    row["portfolio_reasons"] = list(
                        dict.fromkeys([*throttle_reasons[:3], *existing_reasons])
                    )[:4]
                ...
                (
                    ...
                    monitor_rows=[
                        monitor_row
                        for monitor_row in snapshot.get("monitors", [])
                        if str(monitor_row.get("customer") or "").strip()
                        == customer_name
                    ],
                )
                rebalance_history = self._customer_rebalance_history_for_profile(
                    profile
                )
                if rebalance_history:
                    accepted_items = [
                        item
                        for item in items
                        if str(item.customer or "").strip() == customer_name
                        and str(item.status or "").strip().lower() == "accepted"
                    ]
                    ...
                    row["latest_rebalance_evaluation_status"] = latest_rebalance.get(
                        "evaluation_status"
                    )
                    row["latest_rebalance_evaluation_sample_count"] = int(
                        latest_rebalance.get("evaluation_sample_count", 0) or 0
                    )
                    row["latest_rebalance_evaluation_target_count"] = int(
                        latest_rebalance.get("evaluation_target_count", 0) or 0
                    )
                    row["latest_rebalance_evaluation_reasons"] = list(
                        latest_rebalance.get("evaluation_reasons") or []
                    )
                    if (
                        str(latest_rebalance.get("evaluation_status") or "")
                        .strip()
                        .lower()
                        == "degrading"
                    ):
                        ...
                        row["rebalance_guidance_changes"] = []
                        row["rebalance_guidance_reasons"] = list(
                            dict.fromkeys(
                                [
    ...
    async def build_customer_rebalance_preview(
        ...
        *,
        ...
        snapshot = await self.build_effectiveness_analytics(
            db=db, user_id=user_id, customer=customer
        )
        ...
        before_capacity = self._customer_capacity_from_changes(
            base_rows=monitor_rows, changes_by_monitor={}
        )

        default_changes = (
            customer_row.get("rebalance_guidance_changes")
            if isinstance(customer_row, dict)
            else []
        )
        raw_changes = (
            monitor_budget_updates
            if isinstance(monitor_budget_updates, list) and monitor_budget_updates
            else default_changes
        )
        normalized_changes: list[dict[str, Any]] = []
        changes_by_monitor: dict[str, dict[str, int]] = {}
        monitor_by_id = {
            str(row.get("monitor_job_id") or ""): row for row in monitor_rows
        }
        for change in raw_changes:
            if not isinstance(change, dict):
                continue
            ...
            current_budget = self._normalize_budget_config(
                change.get("current_budget")
                if isinstance(change.get("current_budget"), dict)
                else monitor_row.get("autonomy_budget")
            )
            proposed_budget = self._normalize_budget_config(
                change.get("proposed_budget")
                if isinstance(change.get("proposed_budget"), dict)
                else change
            )
            ...
            normalized_changes.append(
                {
                    ...
                    "delta_budget": {
                        key: int(proposed_budget.get(key, 0) or 0)
                        - int(current_budget.get(key, 0) or 0)
                        for key in self.DEFAULT_AUTONOMY_BUDGET.keys()
                    },
                    "reasons": [
                        str(reason).strip()
        ...
        after_capacity = self._customer_capacity_from_changes(
            base_rows=monitor_rows, changes_by_monitor=changes_by_monitor
        )
        return {
            "customer": customer_name,
            "guidance_status": str(
                customer_row.get("rebalance_guidance_status") or "none"
            )
            if isinstance(customer_row, dict)
            else "none",
            "guidance_summary": customer_row.get("rebalance_guidance_summary")
            if isinstance(customer_row, dict)
            else None,
            "guidance_reasons": list(
                customer_row.get("rebalance_guidance_reasons") or []
            )
            if isinstance(customer_row, dict)
            else [],
    ...
    async def recompute_profile(
        ...
        *,
        ...
        pruned_phrases = {
            t: int(s) for t, s in phrase_scores.items() if abs(int(s)) >= 2
        }
        pruned_recommendations = {
            t: int(s) for t, s in recommendation_scores.items() if abs(int(s)) >= 1
        }
        pruned_source_types = {
            t: int(s) for t, s in source_type_scores.items() if abs(int(s)) >= 1
        }
        normalized_outcomes = {
            t: int(v) for t, v in outcome_counters.items() if int(v) != 0
        }
        ...
        existing_res = await db.execute(
            ...
            .where(
                and_(
                    ...
                    if customer
                    else ResearchMonitorProfile.customer.is_(None),
                )
            )
            .limit(1)
        )
