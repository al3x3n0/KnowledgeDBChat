...
async def _async_process_document(task, document_id: str) -> Dict[str, Any]:
    ...
    async with create_celery_session()() as db:
        try:
            ...
            task.update_state(
                state="PROGRESS",
                meta={"current": 1, "total": 4, "status": "Extracting text content"},
            )

            # Step 2: Text chunking
            task.update_state(
                state="PROGRESS",
                meta={"current": 2, "total": 4, "status": "Splitting text into chunks"},
            )
...
async def _async_reprocess_document(
    task, document_id: str, user_id: Optional[str] = None
) -> Dict[str, Any]:
    ...
    async with create_celery_session()() as db:
        try:
            ...
            task.update_state(
                state="PROGRESS",
                meta={"current": 1, "total": 3, "status": "Deleting existing chunks"},
            )
            ...
            success = await document_service.reprocess_document(
                UUID(document_id), db, user_id=UUID(user_id) if user_id else None
            )

            if not success:
                raise ValueError(
                    f"Document {document_id} not found or reprocessing failed"
                )

            task.update_state(
                state="PROGRESS",
                meta={"current": 3, "total": 3, "status": "Reprocessing completed"},
            )
