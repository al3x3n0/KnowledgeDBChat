...
async def test_document_service_get_documents(db_session: AsyncSession):
    ...
    documents, total = await service.get_documents(skip=0, limit=10, db=db_session)
...
async def test_auth_service_authenticate_user(
    ...
    authenticated_user = await service.authenticate_user(
        username="testuser", password="testpassword123", db=db_session
    )
...
async def test_auth_service_authenticate_user_wrong_password(
    ...
    authenticated_user = await service.authenticate_user(
        username="testuser", password="wrongpassword", db=db_session
    )
...
@pytest.mark.asyncio
async def test_auth_service_get_user_by_username(
    db_session: AsyncSession, test_user: User
):
