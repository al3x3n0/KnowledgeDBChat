...
class RedisSubscriber:
    ...
    async def start(self):
        ...
        try:
            ...
            self.running = True
            logger.info(
                "Redis subscriber started for progress forwarding (transcription, summarization, ingestion)"
            )
            ...
            try:
                ...
                if message and message["type"] == "pmessage":
                    ...
                    try:
                        ...
                        channel: str = message.get("channel", "") or message.get(
                            "pattern", ""
                        )
                        ...
                        if (
                            channel.startswith("transcription_progress:")
                            or channel == "transcription_progress:*"
                        ):
                            ...
                            if message_type == "progress":
                                ...
                                await websocket_manager.send_segment(
                                    document_id, segment
                                )
                        elif (
                            channel.startswith("summarization_progress:")
                            or channel == "summarization_progress:*"
                        ):
                            # Forward summarization messages
                            if message_type == "progress":
                                progress = data.get("progress", {})
                                await websocket_manager.send_summarization_progress(
                                    document_id, progress
                                )
                            elif message_type == "complete":
                                result = data.get("result", {})
                                await websocket_manager.send_summarization_complete(
                                    document_id, result
                                )
                            elif message_type == "error":
                                error = data.get("error", "Unknown error")
                                await websocket_manager.send_summarization_error(
                                    document_id, error
                                )
                            elif message_type == "status":
                                status = data.get("status", {})
                                await websocket_manager.send_summarization_status(
                                    document_id, status
                                )
                        elif (
                            channel.startswith("ingestion_progress:")
                            or channel == "ingestion_progress:*"
                        ):
                            # Forward ingestion messages (admin sources)
                            if message_type == "progress":
                                progress = data.get("progress", {})
                                await websocket_manager.send_ingestion_progress(
                                    document_id, progress
                                )
                            elif message_type == "complete":
                                result = data.get("result", {})
                                await websocket_manager.send_ingestion_complete(
                                    document_id, result
                                )
                            elif message_type == "error":
                                error = data.get("error", "Unknown error")
                                await websocket_manager.send_ingestion_error(
                                    document_id, error
                                )
                            elif message_type == "status":
                                status = data.get("status", {})
                                await websocket_manager.send_ingestion_status(
                                    document_id, status
                                )
