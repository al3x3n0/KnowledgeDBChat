...
def test_get_documents_with_pagination(client: TestClient, auth_headers):
    ...
    response = client.get("/api/v1/documents/?skip=0&limit=10", headers=auth_headers)
...
def test_upload_document(client: TestClient, auth_headers, monkeypatch):
    """Test uploading a document."""

    # Object storage (MinIO) is not available in the test environment; stub the
    # upload so the endpoint exercises its own logic rather than making a
    # network call.
    ...
    monkeypatch.setattr(storage_service, "upload_file", _fake_upload_file)

    file_content = b"This is a test document content"
    files = {"file": ("test.txt", BytesIO(file_content), "text/plain")}
    data = {"title": "Test Document", "tags": "test, sample"}
...
def test_get_document_sources(client: TestClient, auth_headers):
    ...
    response = client.get("/api/v1/documents/sources/", headers=auth_headers)
...
def test_sync_document_source(client: TestClient, admin_headers):
    ...
    # Sync the source
    response = client.post(
        f"/api/v1/documents/sources/{source_id}/sync", headers=admin_headers
    )
...
def test_delete_document_source(client: TestClient, admin_headers):
    ...
    response = client.delete(
        f"/api/v1/documents/sources/{source_id}", headers=admin_headers
    )
