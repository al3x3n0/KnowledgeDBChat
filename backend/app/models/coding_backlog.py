...
class CodingBacklogItem(Base):
    ...
    assigned_by_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    orchestrator_job_id = Column(UUID(as_uuid=True), ForeignKey("agent_jobs.id", ondelete="SET NULL"), nullable=True, index=True)
    current_job_id = Column(UUID(as_uuid=True), ForeignKey("agent_jobs.id", ondelete="SET NULL"), nullable=True, index=True)
    latest_apply_job_id = Column(UUID(as_uuid=True), ForeignKey("agent_jobs.id", ondelete="SET NULL"), nullable=True, index=True)
    latest_proposal_id = Column(UUID(as_uuid=True), ForeignKey("code_patch_proposals.id", ondelete="SET NULL"), nullable=True, index=True)
