...
async def create_memory(
    ...
    try:
        memory = await memory_service.create_memory(
            user_id=current_user.id, memory_data=memory_data, db=db
        )
        return memory
...
async def get_memories(
    ...
    memory_types: Optional[str] = Query(
        None, description="Comma-separated memory types"
    ),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
...
async def search_memories(
    ...
    try:
        memories = await memory_service.search_memories(
            user_id=current_user.id, search_request=search_request, db=db
        )
        return memories
    except Exception as e:
        log_error(
            e, context={"user_id": str(current_user.id), "query": search_request.query}
        )
...
async def get_memory(
    ...
    try:
        memory = await memory_service.get_memory_by_id(
            memory_id=memory_id, user_id=current_user.id, db=db
        )
        ...
        log_error(
            e, context={"memory_id": str(memory_id), "user_id": str(current_user.id)}
        )
...
async def extract_memories_from_session(
    ...
    try:
        ...
        return {"message": f"Extracted {len(memories)} memories", "memories": memories}
    except Exception as e:
...
async def generate_memory_summary(
    ...
    try:
        summary = await memory_service.generate_memory_summary(
            user_id=current_user.id, summary_request=summary_request, db=db
        )
        return summary
    except Exception as e:
...
async def get_memory_stats(
    ...
    try:
        stats = await memory_service.get_memory_stats(user_id=current_user.id, db=db)
        return stats
    except Exception as e:
...
async def cleanup_old_memories(
    days_old: int = Query(
        90, ge=1, le=365, description="Delete memories older than this many days"
    ),
