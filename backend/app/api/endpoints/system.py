    ...
    try:
        ...
        if not pongs:
            return HealthCheckServiceResponse(
                status="degraded", error="No workers responded"
            )
        return HealthCheckServiceResponse(
            status="healthy", message=f"{len(pongs)} worker(s) online"
        )
    except Exception as exc:
        return HealthCheckServiceResponse(status="unhealthy", error=str(exc))
...
async def _check_vector_store() -> HealthCheckServiceResponse:
    try:
        ...
        return HealthCheckServiceResponse(
            status="degraded", message="Not initialized yet"
        )
    except Exception as exc:
        return HealthCheckServiceResponse(status="unhealthy", error=str(exc))
    ...
    try:
        if settings.LLM_PROVIDER == "ollama":
            async with httpx.AsyncClient(timeout=2.0) as client:
                ...
                r.raise_for_status()
            return HealthCheckServiceResponse(
                status="healthy", message="Ollama reachable"
            )
        if settings.LLM_PROVIDER == "deepseek":
            if settings.DEEPSEEK_API_KEY:
                return HealthCheckServiceResponse(
                    status="healthy", message="DeepSeek configured"
                )
            return HealthCheckServiceResponse(
                status="degraded", error="DeepSeek API key not set"
            )
        return HealthCheckServiceResponse(
            status="degraded", error=f"Unknown provider: {settings.LLM_PROVIDER}"
        )
    except Exception as exc:
        return HealthCheckServiceResponse(status="unhealthy", error=str(exc))
...
async def get_system_health(
    ...
    if overall != "healthy":
        service_statuses = {key: service.status for key, service in services.items()}
        logger.warning(f"System health is {overall}: {service_statuses}")
...
async def get_unsafe_exec_status(current_user: User = Depends(get_current_user)):
    ...
    enabled_effective = (
        ...
        else bool(getattr(settings, "ENABLE_UNSAFE_CODE_EXECUTION", False))
    )
    backend_override = await get_feature_str("unsafe_code_exec_backend")
    backend = (
        str(
            backend_override
            or getattr(settings, "UNSAFE_CODE_EXEC_BACKEND", "subprocess")
            or "subprocess"
        )
        .strip()
        .lower()
    ...
    docker_image = str(
        image_override
        or getattr(settings, "UNSAFE_CODE_EXEC_DOCKER_IMAGE", "python:3.11-slim")
        or "python:3.11-slim"
    ).strip()
    ...
    if docker_available:
        try:
            code, _out, _err = await _run(
                ["docker", "image", "inspect", docker_image], timeout=1.5
            )
