...
class MinIOStorageService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if not client.bucket_exists(settings.MINIO_BUCKET_NAME):
                ...
                logger.info(
                    f"MinIO bucket already exists: {settings.MINIO_BUCKET_NAME}"
                )
    ...
    async def upload_to_path(
        self, object_path: str, content: bytes, content_type: Optional[str] = None
    ) -> str:
        """
    ...
    def _rewrite_presigned_url_for_proxy(self, url: str) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Rewrote presigned URL from {url[:80]}... to {final_url[:80]}..."
            )
            return final_url

        except Exception as e:
            logger.warning(
                f"Failed to rewrite presigned URL for proxy: {e}, using original URL"
            )
            return url

    async def get_presigned_download_url(
        self, object_path: str, expiry: Optional[int] = None
    ) -> str:
        """
        ...
        try:
            ...
            logger.debug(
                f"Generated presigned URL for {sanitized_path} (original: {object_path}), expires in {expiry_seconds}s"
            )
            return url
            ...
            logger.error(
                f"MinIO S3Error generating presigned URL for path '{object_path}': {e}"
            )
            raise
        except Exception as e:
            logger.error(
                f"Failed to generate presigned URL for path '{object_path}': {e}"
            )
            raise
        ...
        try:
            ...
            if sanitized_path.startswith("documents/"):
                sanitized_path = sanitized_path[
                    10:
                ]  # Remove 'documents/' prefix (10 chars)
            ...
            if e.code == "NoSuchKey":
                logger.warning(
                    f"File not found in MinIO: {object_path} (sanitized: {sanitized_path if 'sanitized_path' in locals() else 'N/A'})"
                )
    ...
    async def file_exists(self, object_path: str) -> bool:
        ...
        try:
            ...
            # Invalid path format
            logger.warning(
                f"Invalid path format for file_exists check: {object_path} - {e}"
            )
            return False
        except Exception as e:
    ...
    def get_file_stream_range(
        self, object_path: str, start: Optional[int] = None, end: Optional[int] = None
    ):
        """
        ...
        try:
            ...
            try:
                ...
                if e.code == "NoSuchKey" and sanitized_path.startswith("documents/"):
                    ...
                    path_without_prefix = sanitized_path[
                        10:
    ...
    async def create_multipart_upload(
        self, object_path: str, content_type: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            if not self.is_multipart_available():
                ...
                logger.info(
                    f"Multipart upload not available, using manual reassembly with ID: {upload_id}"
                )
                return upload_id
            ...
            try:
                ...
                if hasattr(client, "create_multipart_upload"):
                    ...
                    upload_id = f"upload_{uuid.uuid4()}"
                    logger.warning(
                        f"Multipart methods not found despite detection, using manual reassembly: {upload_id}"
                    )
                    return upload_id

                logger.info(
                    f"Created multipart upload for {sanitized_path}, upload_id: {upload_id}"
                )
                return upload_id
            except (AttributeError, S3Error) as e:
                # If multipart upload fails, mark as unavailable and use manual reassembly
                logger.warning(
                    f"Multipart upload failed: {e}. Marking as unavailable and using manual reassembly."
                )
                self._multipart_available = False
                upload_id = f"upload_{uuid.uuid4()}"
                return upload_id
    ...
    async def upload_part(
        self, object_path: str, upload_id: str, part_number: int, data: bytes
    ) -> str:
        """
        ...
        try:
            ...
            etag = (
                ...
                else result.headers.get("ETag", "").strip('"')
            )
            logger.debug(
                f"Uploaded part {part_number} for {sanitized_path}, etag: {etag}"
            )
            return etag
    ...
    async def complete_multipart_upload(
        self, object_path: str, upload_id: str, parts: list
    ) -> None:
        """
        ...
        try:
            ...
            logger.error(
                f"MinIO S3Error completing multipart upload: {e}", exc_info=True
            )
            raise
        except Exception as e:
    ...
    async def copy_file(
        self, source_path: str, dest_path: str, content_type: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if not content_type:
                try:
                    ...
                    logger.warning(
                        f"Could not get source file metadata: {e}, using default content type"
                    )
            ...
            try:
                ...
                copy_source = CopySource(
                    bucket_name=settings.MINIO_BUCKET_NAME, object_name=sanitized_source
                )
