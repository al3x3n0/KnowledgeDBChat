...
async def _async_generate_chat_title(task, session_id: str) -> Dict[str, Any]:
    ...
    async with create_celery_session()() as db:
        try:
            ...
            if len(messages) < 2:
                logger.info(
                    f"Not enough messages in session {session_id} for title generation"
                )
                return {
                    "session_id": session_id,
                    "success": False,
...
Title:"""
            ...
            try:
                ...
                if len(generated_title) > 60:
                    generated_title = generated_title[:60].rsplit(" ", 1)[
                        0
                ...
                logger.info(
                    f"Generated title for session {session_id}: {formatted_title}"
                )
                ...
                logger.error(
                    f"Error generating title with LLM for session {session_id}: {e}",
                    exc_info=True,
                )
            ...
            logger.error(
                f"Error generating chat title for session {session_id}: {e}",
                exc_info=True,
            )
