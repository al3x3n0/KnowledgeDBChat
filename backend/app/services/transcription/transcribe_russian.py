...
class RussianTranscriber:
    ...
    def __init__(
        ...
        logger.info(
            f"Loading Whisper model: {model_size} ({whisper_info['size']}) locally"
        )
        ...
        from .whisper_weights import ensure_local_whisper_weights, get_offline_flag
        ...
        if enable_summarization:
            ...
            logger.info(
                "⏭️  Summarization disabled (transcript only mode) - use --enable-summary to enable"
            )
        ...
        if enable_speaker_diarization:
            ...
            logger.info(
                "⏭️  Speaker diarization disabled - use --enable-speakers to enable"
            )
        ...
        if enable_checkpoints:
            logger.info(
                "💾 Checkpoint system enabled - transcription can be resumed if interrupted"
            )
        else:
            logger.info(
                "⏭️  Checkpoints disabled - use --enable-checkpoints to enable resume functionality"
            )
    ...
    def _load_summarization_models(self, lightweight: bool, model_size: str):
        ...
        try:
            ...
            if lightweight or model_size in ["tiny", "small"]:
                ...
                logger.info(
                    "Loading lightweight Russian summarization model (rut5-small)"
                )
            else:
    ...
    def _load_diarization_model(self):
        ...
        try:
            ...
            logger.info(
                f"Loading pyannote.audio speaker diarization model: {model_id} (token: {'set' if hf_token else 'not set'})"
            )
            if hf_token:
            ...
            logger.warning(
                "⚠️  pyannote.audio not installed. Will try SpeechBrain/local diarization instead"
            )
    ...
    def _perform_local_diarization(self, audio_path: Path) -> dict:
        ...
        try:
            ...
            for s, e, lab in segments:
                ...
                out[(s, e)] = mapping[lab]
            logger.info(
                f"Local diarization detected {len(set(mapping.values()))} speakers"
            )
            return out
        except Exception as e:
    ...
    def extract_audio(
        ...
        try:
            ...
            stream = ffmpeg.output(
                stream, str(output_path), acodec="pcm_s16le", ac=1, ar="16k"
            )
    ...
    def _perform_speaker_diarization(self, audio_path: Path) -> dict:
        ...
        if not self.enable_speaker_diarization:
            logger.info(
                "Diarization disabled by configuration; proceeding without speakers"
            )
            return {}
        if not self.diarization_pipeline:
            ...
            if self._speechbrain_available():
                logger.info(
                    "Using SpeechBrain diarization (ECAPA embeddings + clustering)"
                )
        ...
        try:
            ...
            logger.info(
                f"✓ Found {len(set(speaker_segments.values()))} unique speakers"
            )
    ...
    def _find_speaker_for_segment(
        self, segment_start: float, segment_end: float, speaker_segments: dict
    ) -> str:
        """
    ...
    def transcribe(
        ...
        if stream_output and output_file:
            try:
                stream_file = open(
                    output_file, "w", encoding="utf-8", buffering=1
                )  # Line buffering
        ...
        def show_enhanced_progress():
            ...
            if duration:
                ...
                while not transcription_done.wait(0.1):
                    print(
                        f"\r🎙️  Transcribing... {spinner_chars[i % len(spinner_chars)]}",
                        end="",
                        flush=True,
                    )
                    i += 1
                print("\r🎙️  Transcribing... ✅ Done!     ")
        ...
        logger.info(
            f"✅ Transcription completed. Duration: {result.get('duration', 0):.2f} seconds"
        )
        return result
    ...
    def _transcribe_with_streaming(
        ...
        try:
            ...
            skip_initial = float(
                getattr(app_settings, "TRANSCRIPTION_SKIP_INITIAL_SECONDS", 0.0)
            )
            if skip_initial < 0:
                skip_initial = 0.0
        ...
        if checkpoint_path:
            ...
            if checkpoint_data and self._validate_checkpoint(
                checkpoint_data, audio_path
            ):
                logger.info(
                    f"🔄 Resuming from checkpoint at {checkpoint_data['current_offset']:.1f}s"
                )
            else:
                checkpoint_data = None
        ...
        if checkpoint_data:
            ...
            logger.info(
                f"📂 Resumed: {segments_completed} segments completed, {len(text_parts)} text parts"
            )
        else:
            # Start fresh
        ...
        try:
            ...
            intro_filter_enabled = bool(
                getattr(app_settings, "TRANSCRIPTION_FILTER_INTRO_JUNK", True)
            )
            intro_filter_max_seconds = float(
                getattr(app_settings, "TRANSCRIPTION_INTRO_MAX_SECONDS", 12.0)
            )
            intro_filter_no_speech_prob = float(
                getattr(app_settings, "TRANSCRIPTION_INTRO_NO_SPEECH_PROB", 0.30)
            )
        except Exception:
        ...
        for i in range(start_sample, len(audio), samples_per_segment):
            ...
            segment_mel = whisper.log_mel_spectrogram(segment_audio).to(
                self.model.device
            )
            ...
            real_segment_seconds = float(raw_segment_len) / float(
            ...
            if text:
                ...
                looks_like_intro_credit = ("субтит" in text.lower()) and (
                    len(text) <= 120
                )
                low_conf_intro = (
                    result.no_speech_prob is not None
                    and float(result.no_speech_prob) >= intro_filter_no_speech_prob
                ) and len(text) <= 120
                if (
                    intro_filter_enabled
                    and is_intro_window
                    and (looks_like_intro_credit or low_conf_intro)
                ):
                    logger.info(
                    ...
                    # Create segment info
                    speaker_label = self._find_speaker_for_segment(
        ...
        if not full_text and dropped_intro_segment is not None:
            logger.info(
                "Transcript became empty after intro filtering; restoring first filtered segment"
            )
    ...
    def _stream_with_sentences(
        ...
        try:
            ...
            sentences = [
                m.group(1).strip() for m in sentence_regex.finditer(text.strip())
            ]
            ...
            if len(sentences) == 1:
                ...
                if speaker_segments is not None:
                    sentence_speaker = self._find_speaker_for_segment(
                        start_time, end_time, speaker_segments
                    )
                try:
                    stream_file.write(
                        f"[{sentence_speaker}] {timecode} {sentences[0]}\n"
                    )
                    stream_file.flush()
                except (IOError, OSError) as e:
                ...
                for i, sentence in enumerate(sentences):
                    if total_chars > 0:
                        ...
                        sentence_duration = (
                            len(sentence) / total_chars
                        ) * segment_duration
                    else:
                        # Equal distribution if no characters (safety fallback)
                        sentence_duration = (
                            segment_duration / len(sentences)
                            if len(sentences) > 0
                            else 1.0
                        )
                    ...
                    if speaker_segments is not None:
                        sentence_speaker = self._find_speaker_for_segment(
                            current_time,
                            min(current_time + sentence_duration, end_time),
                            speaker_segments,
                        )
                    try:
                        stream_file.write(
                            f"[{sentence_speaker}] {timecode} {sentence}\n"
                        )
                        stream_file.flush()
                    except (IOError, OSError) as e:
                        logger.error(
                            f"Failed to write sentence {i+1}/{len(sentences)} to stream: {e}"
                        )
            ...
            try:
                if stream_file and not stream_file.closed:
                    timecode = (
                        format_timecode(start_time) if start_time >= 0 else "00:00:00"
                    )
    ...
    def summarize_text(
        self, text: str, max_length: int = 300, min_length: int = 50
    ) -> str:
        """
    ...
    def _fallback_summarize(self, text: str, max_sentences: int = 3) -> str:
        ...
        for i, sentence in enumerate(sentences):
            ...
            words_in_sent = [
                w for w in words_in_sent if w not in self.stop_words and len(w) > 2
            ]

            if not words_in_sent:
                score = 0
            else:
                score = sum(word_freq.get(word, 0) for word in words_in_sent) / len(
                    words_in_sent
                )
    ...
    def process_media_file(
        ...
        try:
            ...
            transcription_result = self.transcribe(
                audio_path, stream_output=stream_output, output_file=stream_file
            )
...
def main():
    ...
    try:
        ...
        if args.resume and not args.stream:
            logger.warning(
                "⚠️  --resume requires --stream for checkpoint support, enabling streaming"
            )
            args.stream = True
        ...
        transcript, summary = transcriber.process_media_file(
            args.input, args.output, stream_output=args.stream
        )
